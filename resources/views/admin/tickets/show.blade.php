...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div class="lg:col-span-3 space-y-6">
                        ...
                        <div class="flex items-center gap-2.5">
                            ...
                            <div>
                                <div class="flex items-center gap-2">
                                    ...
                                    @else
                                        <span class="px-2 py-0.2 rounded-full text-[9px] font-semibold bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 border border-slate-200 dark:border-slate-700">Gym Owner</span>
                                    @endif
                                </div>
                                <span class="text-[10px] text-slate-400 dark:text-slate-500 block">{{ $reply->created_at->format('d M Y, h:i A') }} ({{ $reply->created_at->diffForHumans() }})</span>
                            </div>
                        </div>
                    </div>
            ...
            <div class="p-6 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xs dark:shadow-xl space-y-4 transition-colors">
                ...
                <form action="{{ route('admin.tickets.reply', $ticket->id) }}" method="POST" enctype="multipart/form-data" class="space-y-4 text-xs">
                    ...
                    <div>
                        <label class="font-bold text-slate-700 dark:text-slate-300 block mb-1.5">Response Message <span class="text-red-500">*</span></label>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div class="space-y-1">
                            <label class="font-bold text-slate-700 dark:text-slate-300">Attachment (Optional)</label>
                            <input type="file" name="attachment" 
                                   class="w-full px-3 py-1.5 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-800 text-slate-600 dark:text-slate-400 file:mr-3 file:py-1 file:px-2.5 file:rounded-lg file:border-0 file:text-xs file:font-semibold file:bg-slate-100 dark:file:bg-slate-800 file:text-slate-700 dark:file:text-slate-300 hover:file:bg-slate-200 dark:hover:file:bg-slate-700 cursor-pointer text-xs">
                        </div>
                    </div>
        ...
        <div class="space-y-6">
            ...
            <div class="p-5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xs dark:shadow-xl space-y-4 text-xs transition-colors">
                <h4 class="text-xs font-bold text-slate-900 dark:text-white uppercase tracking-wider border-b border-slate-200 dark:border-slate-800 pb-2.5">
                    Ticket Controls
                </h4>
                ...
                <form action="{{ route('admin.tickets.status', $ticket->id) }}" method="POST" class="space-y-3">
                    @csrf
                    <div class="space-y-1">
                        <label class="text-[10px] text-slate-500 dark:text-slate-400 uppercase font-bold">Status</label>
                    ...
                    <div class="space-y-1">
                        <label class="text-[10px] text-slate-500 dark:text-slate-400 uppercase font-bold">Priority</label>
                        <select name="priority" class="w-full px-3 py-2 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-800 text-slate-900 dark:text-white text-xs focus:border-red-500 focus:outline-none cursor-pointer">
                    ...
                    <button type="submit" class="w-full py-2 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-800 dark:bg-slate-800 dark:hover:bg-slate-700 dark:text-slate-200 text-xs font-bold border border-slate-300 dark:border-slate-700 transition-colors cursor-pointer">
            ...
            <div class="p-5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xs dark:shadow-xl space-y-4 text-xs transition-colors">
                ...
                <div class="space-y-2.5">
                    <div>
                        <span class="text-[10px] text-slate-500 dark:text-slate-400 uppercase block font-semibold">Gym / Tenant</span>
                        <p class="font-bold text-slate-900 dark:text-white text-sm">{{ $ticket->tenant->name ?? 'N/A' }}</p>
                        <p class="text-[10px] text-slate-400 font-mono">{{ $ticket->tenant->slug ?? '' }}</p>
                    </div>

                    <div>
                        <span class="text-[10px] text-slate-500 dark:text-slate-400 uppercase block font-semibold">Active SaaS Plan</span>
                        <span class="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 border border-indigo-500/20 inline-block mt-0.5">
                            {{ $ticket->tenant->activeSubscription?->plan?->name ?? 'No Active Plan' }}
                        </span>
                    </div>

                    <div>
                        <span class="text-[10px] text-slate-500 dark:text-slate-400 uppercase block font-semibold">Ticket Creator</span>
                        <p class="font-bold text-slate-900 dark:text-white">{{ $ticket->user->name ?? 'N/A' }}</p>
                        <p class="text-[11px] text-slate-500 dark:text-slate-400">{{ $ticket->user->email ?? 'N/A' }}</p>
                        @if($ticket->user?->phone)
                            <p class="text-[11px] text-slate-500 dark:text-slate-400">{{ $ticket->user->phone }}</p>
                        @endif
                    </div>
                    ...
                    <div class="border-t border-slate-200 dark:border-slate-800 pt-3 flex items-center justify-between">
                        <form action="{{ route('admin.gyms.impersonate', $ticket->tenant_id) }}" method="POST" class="w-full">
                            @csrf
                            <button type="submit" class="w-full py-2 rounded-xl bg-indigo-600/10 hover:bg-indigo-600/20 text-indigo-600 dark:text-indigo-400 font-bold text-xs border border-indigo-500/20 transition-colors flex items-center justify-center gap-1.5 cursor-pointer">
