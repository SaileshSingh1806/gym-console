<x-app-layout header="Member Profile">
    ...
    <div x-data="{
        ...
        <!-- ========================================================================= -->
        <!-- MODAL 7: EDIT MEMBER DETAILS (Full Profile & Photos)                      -->
        <!-- ========================================================================= -->
        <div x-show="showEditModal" 
             x-transition:enter="transition ease-out duration-300"
             x-transition:enter-start="opacity-0 scale-95"
                ...
                <div class="flex items-center justify-between border-b border-slate-800/80 pb-4">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <div class="flex items-center gap-2">
                                <h3 class="text-lg font-black text-white">Edit Member Details</h3>
                                <span class="px-2 py-0.5 rounded-md bg-slate-800 text-amber-400 text-xs font-mono font-bold border border-slate-700">{{ $member->member_code }}</span>
                            </div>
                            <p class="text-xs text-slate-400">Update personal details, profile picture, body metrics, and membership status</p>
                        </div>
                    </div>
                ...
                <form action="{{ route('app.members.update', $member->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6 text-xs">
                    ...
                    <div class="p-5 rounded-2xl bg-slate-950/60 border border-slate-800 space-y-4">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            ...
                            <div>
                                <label class="block font-bold text-slate-300 mb-1.5">Current Weight (kg)</label>
                                <input type="number" step="0.1" name="weight" value="{{ $member->metadata['weight'] ?? '' }}" placeholder="e.g. 72.5" class="w-full px-3.5 py-2.5 rounded-xl bg-slate-900 border border-slate-700 text-white placeholder-slate-500 focus:border-indigo-500 focus:outline-none">
                            </div>
                    ...
                    <div class="p-5 rounded-2xl bg-slate-950/60 border border-slate-800 space-y-4">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            <div>
                                <label class="block font-bold text-slate-300 mb-1.5">Member Status <span class="text-rose-500">*</span></label>
                                <select name="status" class="w-full px-3.5 py-2.5 rounded-xl bg-slate-900 border border-slate-700 text-white focus:border-indigo-500 focus:outline-none font-bold">
                                    <option value="ACTIVE" class="text-emerald-400" {{ $member->status === 'ACTIVE' ? 'selected' : '' }}>🟢 Active</option>
                                    <option value="INACTIVE" class="text-slate-400" {{ $member->status === 'INACTIVE' ? 'selected' : '' }}>⚪ Inactive</option>
                                    <option value="SUSPENDED" class="text-rose-400" {{ $member->status === 'SUSPENDED' ? 'selected' : '' }}>🔴 Suspended / Frozen</option>
                                    <option value="EXPIRED" class="text-orange-400" {{ $member->status === 'EXPIRED' ? 'selected' : '' }}>🟠 Expired</option>
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                <select name="membership_plan_id" class="w-full px-3.5 py-2.5 rounded-xl bg-slate-900 border border-slate-700 text-white focus:border-indigo-500 focus:outline-none">
                        ...
                        <div>
                            ...
                            <textarea name="notes" rows="2" placeholder="Optional notes regarding medical history or special arrangements" class="w-full px-3.5 py-2.5 rounded-xl bg-slate-900 border border-slate-700 text-white placeholder-slate-500 focus:border-indigo-500 focus:outline-none">{{ $member->notes }}</textarea>
                        </div>
                    </div>
