    ...
    <div x-data="{ 
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                <h1 class="text-2xl font-black text-slate-900 dark:text-white tracking-tight flex items-center gap-2">
                    ...
                    @if(auth()->user()->tenant?->isSubscriptionExpired())
                        <span class="px-2.5 py-0.5 rounded-full bg-red-100 dark:bg-red-500/10 text-red-700 dark:text-red-400 border border-red-300 dark:border-red-500/20 text-xs font-bold uppercase tracking-wider">EXPIRED</span>
                    @else
                        <span class="px-2.5 py-0.5 rounded-full bg-emerald-100 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border border-emerald-300 dark:border-emerald-500/20 text-xs font-bold uppercase tracking-wider">ACTIVE</span>
                    @endif
                </h1>
            ...
            <div class="flex items-center gap-2">
                <div class="px-3.5 py-1.5 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-xs font-bold text-slate-700 dark:text-slate-300 flex items-center gap-2 shadow-xs">
                    <span class="w-2 h-2 rounded-full bg-emerald-500"></span>
                    <span>Branch: <strong class="text-slate-900 dark:text-white font-extrabold">{{ $branch->name ?? 'Main Branch' }}</strong></span>
                </div>
            </div>
        </div>
        ...
        <div class="flex items-center gap-2.5 overflow-x-auto pb-1 text-xs font-bold no-scrollbar">
            <!-- 1. Add Member -->
            <a href="{{ route('app.members.create') }}" class="px-3.5 py-2 rounded-xl bg-emerald-50 hover:bg-emerald-100 text-emerald-800 border border-emerald-300 dark:bg-emerald-500/15 dark:hover:bg-emerald-500/25 dark:text-emerald-400 dark:border-emerald-500/30 flex items-center gap-2 whitespace-nowrap transition-all shadow-xs">
            ...
            <!-- 2. Mark Attendance -->
            <a href="{{ route('app.attendance.index') }}" class="px-3.5 py-2 rounded-xl bg-blue-50 hover:bg-blue-100 text-blue-800 border border-blue-300 dark:bg-blue-500/15 dark:hover:bg-blue-500/25 dark:text-blue-400 dark:border-blue-500/30 flex items-center gap-2 whitespace-nowrap transition-all shadow-xs">
            ...
            <!-- 3. Record Payment -->
            <a href="{{ route('app.payments.index') }}" class="px-3.5 py-2 rounded-xl bg-purple-50 hover:bg-purple-100 text-purple-800 border border-purple-300 dark:bg-purple-500/15 dark:hover:bg-purple-500/25 dark:text-purple-400 dark:border-purple-500/30 flex items-center gap-2 whitespace-nowrap transition-all shadow-xs">
            ...
            <a href="{{ route('app.leads.index') }}" class="px-3.5 py-2 rounded-xl bg-amber-50 hover:bg-amber-100 text-amber-800 border border-amber-300 dark:bg-amber-500/15 dark:hover:bg-amber-500/25 dark:text-amber-400 dark:border-amber-500/30 flex items-center gap-2 whitespace-nowrap transition-all shadow-xs">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z"/></svg>
                <span>Add Lead</span>
            </a>
            @endif
            ...
            <a href="{{ route('app.inventory.index') }}" class="px-3.5 py-2 rounded-xl bg-rose-50 hover:bg-rose-100 text-rose-800 border border-rose-300 dark:bg-rose-500/15 dark:hover:bg-rose-500/25 dark:text-rose-400 dark:border-rose-500/30 flex items-center gap-2 whitespace-nowrap transition-all shadow-xs">
            ...
            <a href="{{ route('app.expenses.index') }}" class="px-3.5 py-2 rounded-xl bg-indigo-50 hover:bg-indigo-100 text-indigo-800 border border-indigo-300 dark:bg-indigo-500/15 dark:hover:bg-indigo-500/25 dark:text-indigo-400 dark:border-indigo-500/30 flex items-center gap-2 whitespace-nowrap transition-all shadow-xs">
            ...
            <a href="{{ route('app.settings.index', ['tab' => 'devices']) }}" class="px-3.5 py-2 rounded-xl bg-teal-50 hover:bg-teal-100 text-teal-800 border border-teal-300 dark:bg-teal-500/15 dark:hover:bg-teal-500/25 dark:text-teal-400 dark:border-teal-500/30 flex items-center gap-2 whitespace-nowrap transition-all shadow-xs">
            ...
            <!-- 8. Tutorials -->
            <a href="{{ route('app.settings.index') }}" class="px-3.5 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-800 border border-slate-300 dark:bg-slate-800 dark:hover:bg-slate-700 dark:text-slate-300 dark:border-slate-700/60 flex items-center gap-2 whitespace-nowrap transition-all shadow-xs">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
            ...
            <a href="{{ route('app.members.index', ['status' => 'ACTIVE']) }}" 
               class="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/90 shadow-xs flex items-center gap-3.5 hover:border-emerald-500/50 hover:shadow-md transition-all group">
                <div class="w-11 h-11 rounded-xl bg-emerald-100 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 flex items-center justify-center shrink-0 border border-emerald-200 dark:border-emerald-500/20 group-hover:scale-105 transition-transform">
                ...
                <div>
                    <div class="text-2xl font-black text-slate-900 dark:text-white leading-none group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors">{{ $metrics['active_members'] }}</div>
                    <div class="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mt-1">Active Members</div>
                </div>
            </a>

            <!-- 2. Expired Members (Links to /app/members?status=EXPIRED) -->
            <a href="{{ route('app.members.index', ['status' => 'EXPIRED']) }}" 
               class="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/90 shadow-xs flex items-center gap-3.5 hover:border-red-500/50 hover:shadow-md transition-all group">
                <div class="w-11 h-11 rounded-xl bg-red-100 dark:bg-red-500/10 text-red-700 dark:text-red-400 flex items-center justify-center shrink-0 border border-red-200 dark:border-red-500/20 group-hover:scale-105 transition-transform">
                ...
                <div>
                    <div class="text-2xl font-black text-slate-900 dark:text-white leading-none group-hover:text-red-600 dark:group-hover:text-red-400 transition-colors">{{ $metrics['expired_members'] }}</div>
                    <div class="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mt-1">Expired Members</div>
                </div>
            </a>
            ...
            <button type="button" 
                    ...
                    class="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/90 shadow-xs flex items-center gap-3.5 hover:border-rose-500/50 hover:shadow-md transition-all group text-left cursor-pointer">
                <div class="w-11 h-11 rounded-xl bg-rose-100 dark:bg-rose-500/10 text-rose-700 dark:text-rose-400 flex items-center justify-center shrink-0 border border-rose-200 dark:border-rose-500/20 group-hover:scale-105 transition-transform">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg>
                </div>
                <div>
                    <div class="text-2xl font-black text-slate-900 dark:text-white leading-none group-hover:text-rose-600 dark:group-hover:text-rose-400 transition-colors">{{ $metrics['churn_risk'] }}</div>
                    <div class="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mt-1">Churn Risk</div>
                </div>
            </button>
            ...
            <button type="button" 
                    ...
                    class="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/90 shadow-xs flex items-center gap-3.5 relative overflow-hidden hover:border-amber-500/50 hover:shadow-md transition-all group text-left cursor-pointer">
                ...
                <div>
                    <div class="text-2xl font-black text-slate-900 dark:text-white leading-none group-hover:text-amber-600 dark:group-hover:text-amber-400 transition-colors">{{ $metrics['expiring_soon'] }}</div>
                    <div class="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mt-1">Expiring Soon</div>
                </div>
            </button>
            ...
            <a href="{{ route('app.members.index', ['filter' => 'dormant']) }}" 
                ...
                <div>
                    <div class="text-2xl font-black text-slate-900 dark:text-white leading-none group-hover:text-amber-600 dark:group-hover:text-amber-400 transition-colors">{{ $metrics['dormant_members'] }}</div>
                    <div class="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mt-1">Dormant Members</div>
                </div>
            </a>
            ...
            <button type="button" 
                    ...
                    class="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/90 shadow-xs flex items-center gap-3.5 hover:border-pink-500/50 hover:shadow-md transition-all group text-left cursor-pointer">
                <div class="w-11 h-11 rounded-xl bg-pink-100 dark:bg-pink-500/10 text-pink-700 dark:text-pink-400 flex items-center justify-center shrink-0 border border-pink-200 dark:border-pink-500/20 group-hover:scale-105 transition-transform">
                    <span class="text-xl">🎂</span>
                </div>
                <div>
                    <div class="text-2xl font-black text-slate-900 dark:text-white leading-none group-hover:text-pink-600 dark:group-hover:text-pink-400 transition-colors">{{ $metrics['birthdays_today'] }}</div>
                    <div class="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mt-1">Birthdays Today</div>
                </div>
            </button>
        ...
        <div class="space-y-3">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <a href="{{ route('app.pt.index', ['tab' => 'packages', 'status' => 'ACTIVE']) }}" class="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/90 shadow-xs flex items-center gap-3.5 hover:border-purple-500/40 hover:shadow-md transition-all">
                    <div class="w-11 h-11 rounded-xl bg-purple-100 dark:bg-purple-500/10 text-purple-700 dark:text-purple-400 flex items-center justify-center shrink-0 border border-purple-200 dark:border-purple-500/20">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M13 10V3L4 14h7v7l9-11h-7z"/></svg>
                    </div>
                    <div>
                        <div class="text-2xl font-black text-slate-900 dark:text-white leading-none">{{ $metrics['active_pt'] }}</div>
                        <div class="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mt-1">Active PT Packages</div>
                    </div>
                </a>

                <a href="{{ route('app.pt.index', ['tab' => 'packages', 'status' => 'EXPIRED']) }}" class="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/90 shadow-xs flex items-center gap-3.5 hover:border-red-500/40 hover:shadow-md transition-all">
                    <div class="w-11 h-11 rounded-xl bg-red-100 dark:bg-red-500/10 text-red-700 dark:text-red-400 flex items-center justify-center shrink-0 border border-red-200 dark:border-red-500/20">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                    </div>
                    <div>
                        <div class="text-2xl font-black text-slate-900 dark:text-white leading-none">{{ $metrics['expired_pt'] }}</div>
                        <div class="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mt-1">Expired PT Packages</div>
                    </div>
                </a>

                <a href="{{ route('app.pt.index', ['tab' => 'packages', 'status' => 'COMPLETED']) }}" class="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/90 shadow-xs flex items-center gap-3.5 hover:border-amber-500/40 hover:shadow-md transition-all">
                    <div class="w-11 h-11 rounded-xl bg-amber-100 dark:bg-amber-500/10 text-amber-800 dark:text-amber-400 flex items-center justify-center shrink-0 border border-amber-200 dark:border-amber-500/20">
                    ...
                    <div>
                        <div class="text-2xl font-black text-slate-900 dark:text-white leading-none">{{ $metrics['exhausted_pt'] }}</div>
                        <div class="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mt-1">Exhausted PT Packages</div>
                    </div>
                </a>
            </div>
        </div>
        ...
        <div class="space-y-3">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
                <a href="{{ route('app.leads.index') }}" class="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/90 shadow-xs flex items-center gap-3.5 hover:border-blue-500/40 hover:shadow-md transition-all">
                    <div class="w-11 h-11 rounded-xl bg-blue-100 dark:bg-blue-500/10 text-blue-700 dark:text-blue-400 flex items-center justify-center shrink-0 border border-blue-200 dark:border-blue-500/20">
                    ...
                    <div>
                        <div class="text-2xl font-black text-slate-900 dark:text-white leading-none">{{ $metrics['active_leads'] }}</div>
                        <div class="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mt-1">Active Leads</div>
                    </div>
                </a>

                <a href="{{ route('app.leads.index') }}" class="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/90 shadow-xs flex items-center gap-3.5 hover:border-emerald-500/40 hover:shadow-md transition-all">
                    <div class="w-11 h-11 rounded-xl bg-emerald-100 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 flex items-center justify-center shrink-0 border border-emerald-200 dark:border-emerald-500/20">
                    ...
                    <div>
                        <div class="text-2xl font-black text-slate-900 dark:text-white leading-none">{{ $metrics['today_enquiries'] }}</div>
                        <div class="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mt-1">Today's Enquiries</div>
                    </div>
                </a>

                <a href="{{ route('app.leads.index') }}" class="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/90 shadow-xs flex items-center gap-3.5 hover:border-cyan-500/40 hover:shadow-md transition-all">
                    <div class="w-11 h-11 rounded-xl bg-cyan-100 dark:bg-cyan-500/10 text-cyan-700 dark:text-cyan-400 flex items-center justify-center shrink-0 border border-cyan-200 dark:border-cyan-500/20">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/></svg>
                    </div>
                    <div>
                        <div class="text-2xl font-black text-slate-900 dark:text-white leading-none">{{ $metrics['today_followups'] }}</div>
                        <div class="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mt-1">Follow-ups Today</div>
                    </div>
                </a>

                <a href="{{ route('app.leads.index') }}" class="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/90 shadow-xs flex items-center gap-3.5 relative overflow-hidden hover:border-rose-500/40 hover:shadow-md transition-all">
                    @if($metrics['overdue_followups'] > 0)
                        <span class="absolute top-2 right-2 px-1.5 py-0.2 text-[8px] font-extrabold uppercase bg-red-100 dark:bg-red-500/20 text-red-700 dark:text-red-400 border border-red-300 dark:border-red-500/30 rounded">● URGENT</span>
                    @endif
                    <div class="w-11 h-11 rounded-xl bg-rose-100 dark:bg-rose-500/10 text-rose-700 dark:text-rose-400 flex items-center justify-center shrink-0 border border-rose-200 dark:border-rose-500/20">
                    ...
                    <div>
                        <div class="text-2xl font-black text-slate-900 dark:text-white leading-none">{{ $metrics['overdue_followups'] }}</div>
                        <div class="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mt-1">Overdue Follow-ups</div>
                    </div>
                </a>

                <a href="{{ route('app.leads.index') }}" class="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/90 shadow-xs flex items-center gap-3.5 hover:border-teal-500/40 hover:shadow-md transition-all">
                    <div class="w-11 h-11 rounded-xl bg-teal-100 dark:bg-teal-500/10 text-teal-700 dark:text-teal-400 flex items-center justify-center shrink-0 border border-teal-200 dark:border-teal-500/20">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                    </div>
                    <div>
                        <div class="text-2xl font-black text-slate-900 dark:text-white leading-none">{{ $metrics['upcoming_trials'] }}</div>
                        <div class="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mt-1">Upcoming Trials</div>
                    </div>
                </a>
            </div>
        </div>
        ...
        <div class="space-y-6">
            ...
            <div class="p-6 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xs space-y-6">
                <div class="flex items-center justify-between border-b border-slate-200 dark:border-slate-800/80 pb-4">
                    <div class="flex items-center gap-2.5">
                        <div class="w-8 h-8 rounded-lg bg-indigo-100 dark:bg-indigo-500/10 text-indigo-700 dark:text-indigo-400 border border-indigo-200 dark:border-indigo-500/20 flex items-center justify-center">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                        </div>
                        <h3 class="font-extrabold text-slate-900 dark:text-white text-base">Financial Overview</h3>
                    </div>
                    <select class="bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-800 text-xs font-bold text-slate-700 dark:text-slate-300 rounded-xl px-3 py-1.5 focus:outline-none focus:ring-2 focus:ring-indigo-500">
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                    ...
                    <div class="p-4 rounded-xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 flex flex-col justify-between">
                        <div>
                            <div class="w-8 h-8 rounded-lg bg-indigo-100 dark:bg-indigo-500/10 text-indigo-700 dark:text-indigo-400 flex items-center justify-center mb-2.5">
                            ...
                            <div class="text-xl font-black text-slate-900 dark:text-white leading-none">{{ $currency }}{{ number_format($metrics['month_billed_total'] ?? ($metrics['month_revenue'] + $metrics['all_dues_remaining']), 2) }}</div>
                            <div class="text-[9px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mt-1">Total Packages Billed</div>
                        </div>
                        <a href="{{ route('app.memberships.index') }}" class="text-[10px] text-indigo-600 dark:text-indigo-400 hover:underline font-bold mt-3">View More →</a>
                    </div>

                    <!-- 2. Product Sales -->
                    <div class="p-4 rounded-xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 flex flex-col justify-between">
                        <div>
                            <div class="w-8 h-8 rounded-lg bg-purple-100 dark:bg-purple-500/10 text-purple-700 dark:text-purple-400 flex items-center justify-center mb-2.5">
                            ...
                            <div class="text-xl font-black text-slate-900 dark:text-white leading-none">0.00</div>
                            <div class="text-[9px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mt-1">Product Sales</div>
                        </div>
                        <a href="{{ route('app.inventory.index') }}" class="text-[10px] text-purple-600 dark:text-purple-400 hover:underline font-bold mt-3">View More →</a>
                    </div>

                    <!-- 3. Payments Collected -->
                    <div class="p-4 rounded-xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 flex flex-col justify-between">
                        <div>
                            <div class="w-8 h-8 rounded-lg bg-emerald-100 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 flex items-center justify-center mb-2.5">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                            </div>
                            <div class="text-xl font-black text-emerald-600 dark:text-emerald-400 leading-none">{{ $currency }}{{ number_format($metrics['month_revenue'], 2) }}</div>
                            <div class="text-[9px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mt-1">Payments Collected</div>
                        </div>
                        <a href="{{ route('app.payments.index') }}" class="text-[10px] text-emerald-600 dark:text-emerald-400 hover:underline font-bold mt-3">View More →</a>
                    </div>

                    <!-- 4. Payments Pending -->
                    <div class="p-4 rounded-xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 flex flex-col justify-between">
                        <div>
                            <div class="w-8 h-8 rounded-lg bg-amber-100 dark:bg-amber-500/10 text-amber-800 dark:text-amber-400 flex items-center justify-center mb-2.5">
                    ...
                    <div class="p-4 rounded-xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 flex flex-col justify-between">
                        <div>
                            <div class="w-8 h-8 rounded-lg bg-blue-100 dark:bg-blue-500/10 text-blue-700 dark:text-blue-400 flex items-center justify-center mb-2.5">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z"/></svg>
                            </div>
                    ...
                    <div class="p-4 rounded-xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 flex flex-col justify-between">
                        <div>
                            ...
                            <div class="text-xl font-black text-slate-900 dark:text-white leading-none">{{ $metrics['renewals_count'] }}</div>
                            <div class="text-[9px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mt-1">Renewals</div>
                        </div>
                        <a href="{{ route('app.memberships.index') }}" class="text-[10px] text-purple-600 dark:text-purple-400 hover:underline font-bold mt-3">View More →</a>
                    </div>
                </div>
            </div>
            ...
            <div class="p-6 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xs space-y-4">
                <div class="flex items-center gap-2">
                    ...
                    <h3 class="font-extrabold text-slate-900 dark:text-white text-base">Dues Overview</h3>
                </div>
                ...
                <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <a href="{{ route('app.members.index', ['filter' => 'due']) }}" class="p-4 rounded-xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 flex items-center justify-between hover:border-indigo-500/40 hover:bg-slate-100/50 dark:hover:bg-slate-800/60 transition-all cursor-pointer group shadow-xs">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <div class="text-xl font-black text-slate-900 dark:text-white">{{ $currency }}{{ number_format($metrics['all_dues_remaining'], 0) }}</div>
                    ...
                    <a href="{{ route('app.members.index', ['filter' => 'due']) }}" class="p-4 rounded-xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 flex items-center justify-between hover:border-amber-500/40 hover:bg-slate-100/50 dark:hover:bg-slate-800/60 transition-all cursor-pointer group shadow-xs">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <div class="text-xl font-black text-slate-900 dark:text-white">0</div>
                                <div class="text-[9px] font-bold text-slate-500 dark:text-slate-400 uppercase">Due Today</div>
                            </div>
                        </div>
                    ...
                    <a href="{{ route('app.members.index', ['filter' => 'due']) }}" class="p-4 rounded-xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 flex items-center justify-between hover:border-red-500/40 hover:bg-slate-100/50 dark:hover:bg-slate-800/60 transition-all cursor-pointer group shadow-xs">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <div class="text-xl font-black text-slate-900 dark:text-white">{{ $currency }}{{ number_format($metrics['all_dues_remaining'] > 0 ? $metrics['all_dues_remaining'] : 0, 0) }}</div>
        ...
        <div class="p-6 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xs space-y-6">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-800/80 pb-4">
                <div class="flex items-center gap-2.5">
                    <svg class="w-5 h-5 text-indigo-600 dark:text-indigo-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M7 12l3-3 3 3 4-4M8 21l4-4 4 4M3 4h18M4 4h16v12a1 1 0 01-1 1H5a1 1 0 01-1-1V4z"/></svg>
                    <h3 class="font-extrabold text-slate-900 dark:text-white text-base">Trends & Insights</h3>
                ...
                <div class="flex items-center gap-3">
                    <div class="flex items-center bg-slate-100 dark:bg-slate-950 p-1 rounded-xl border border-slate-200 dark:border-slate-800 text-xs font-bold">
                        <button @click="trendPeriod = '30d'" :class="trendPeriod === '30d' ? 'bg-indigo-600 text-white shadow-xs' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'" class="px-3 py-1 rounded-lg transition-all cursor-pointer">30D</button>
                        <button @click="trendPeriod = '90d'" :class="trendPeriod === '90d' ? 'bg-indigo-600 text-white shadow-xs' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'" class="px-3 py-1 rounded-lg transition-all cursor-pointer">90D</button>
                        <button @click="trendPeriod = '6m'" :class="trendPeriod === '6m' ? 'bg-indigo-600 text-white shadow-xs' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'" class="px-3 py-1 rounded-lg transition-all cursor-pointer">6M</button>
                        <button @click="trendPeriod = '1y'" :class="trendPeriod === '1y' ? 'bg-indigo-600 text-white shadow-xs' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'" class="px-3 py-1 rounded-lg transition-all cursor-pointer">1Y</button>
                    </div>
                    <button type="button" class="px-3.5 py-1.5 rounded-xl bg-gradient-to-r from-purple-600 to-indigo-600 text-white font-bold text-xs flex items-center gap-1.5 shadow-md shadow-purple-600/20 hover:opacity-95 cursor-pointer">
                ...
                <div class="p-5 rounded-xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 space-y-3">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <span class="text-2xl font-black text-slate-900 dark:text-white">{{ $currency }}{{ number_format($metrics['month_revenue'], 2) }}</span>
                        </div>
                ...
                <div class="p-5 rounded-xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 space-y-3">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <span class="text-2xl font-black text-slate-900 dark:text-white">{{ $metrics['active_members'] }} active</span>
                        </div>
                        <span class="px-2 py-0.5 rounded-full bg-emerald-100 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-500/20 text-xs font-bold">
                ...
                <div class="p-5 rounded-xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 space-y-3">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <span class="text-2xl font-black text-slate-900 dark:text-white">{{ $metrics['today_attendance'] }}/day</span>
                        </div>
                ...
                <div class="p-5 rounded-xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 space-y-3">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <span class="text-2xl font-black text-slate-900 dark:text-white">{{ $currency }}{{ number_format($metrics['net_profit'], 2) }}</span>
                        </div>
                ...
                <div>
                    ...
                    <div class="space-y-2 max-h-56 overflow-y-auto">
                        @forelse($metrics['expiring_members_list'] as $exp)
                            <div class="p-2.5 rounded-xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800/80 flex items-center justify-between text-xs">
                                <div class="flex items-center gap-2.5">
                                    <div class="w-7 h-7 rounded-full bg-amber-100 dark:bg-amber-500/20 text-amber-800 dark:text-amber-400 flex items-center justify-center font-bold text-[10px]">
                                        {{ substr($exp->member?->first_name ?? 'M', 0, 1) }}
                                    </div>
                                    <div>
                                        <div class="font-bold text-slate-900 dark:text-white leading-tight">{{ $exp->member?->full_name }}</div>
                ...
                <div>
                    ...
                    <div class="space-y-2 max-h-56 overflow-y-auto">
                        @forelse($metrics['birthday_members_list'] as $bday)
                            <div class="p-2.5 rounded-xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800/80 flex items-center justify-between text-xs">
                                <div class="flex items-center gap-2.5">
                                    <div class="w-7 h-7 rounded-full bg-pink-100 dark:bg-pink-500/20 text-pink-700 dark:text-pink-400 flex items-center justify-center font-bold text-[10px]">
                                        🎂
                                    </div>
                                    <div>
                                        <div class="font-bold text-slate-900 dark:text-white leading-tight">{{ $bday->full_name }}</div>
                                        <div class="text-[10px] text-slate-500 dark:text-slate-400">{{ $bday->phone ?? 'Member' }}</div>
                                    </div>
                                </div>
                                @if($bday->phone)
                ...
                <div>
                    ...
                    <div class="space-y-2 max-h-56 overflow-y-auto">
                        @forelse($metrics['recently_expired_list'] as $recExp)
                            <div class="p-2.5 rounded-xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800/80 flex items-center justify-between text-xs">
                                <div class="flex items-center gap-2.5">
                                    <div class="w-7 h-7 rounded-full bg-red-100 dark:bg-red-500/20 text-red-700 dark:text-red-400 flex items-center justify-center font-bold text-[10px]">
                                        {{ substr($recExp->member?->first_name ?? 'M', 0, 1) }}
                                    </div>
                                    <div>
                                        <div class="font-bold text-slate-900 dark:text-white leading-tight">{{ $recExp->member?->full_name }}</div>
                                        <div class="text-[10px] text-slate-500 dark:text-slate-400">{{ $recExp->plan?->name ?? 'Membership' }}</div>
                                    </div>
                                </div>
                ...
                <div>
                    ...
                    <div class="space-y-2 max-h-56 overflow-y-auto">
                        @forelse($metrics['recent_payments'] as $pay)
                            <div class="p-2.5 rounded-xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800/80 flex items-center justify-between text-xs">
                                <div>
                                    <div class="font-bold text-indigo-700 dark:text-indigo-400 leading-tight">{{ $pay->member?->full_name ?? 'Member Payment' }}</div>
                                    <div class="text-[10px] text-slate-500 dark:text-slate-400">{{ $pay->membership?->plan?->name ?? 'Plan Fee' }}</div>
                                </div>
                ...
                <div>
                    ...
                    <div class="space-y-2 max-h-56 overflow-y-auto">
                        @forelse($metrics['recent_members_list'] as $rm)
                            <div class="p-2.5 rounded-xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800/80 flex items-center justify-between text-xs">
                                <div>
                                    <div class="font-bold text-blue-700 dark:text-blue-400 leading-tight">{{ $rm->full_name }}</div>
                                    <div class="text-[10px] text-slate-500 dark:text-slate-400">{{ $rm->phone }}</div>
                                </div>
                                <div class="text-right">
                ...
                <div>
                    ...
                    <div class="space-y-2 max-h-56 overflow-y-auto">
                        @forelse($metrics['lead_followups_list'] as $lead)
                            <div class="p-2.5 rounded-xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800/80 flex items-center justify-between text-xs">
                                <div>
                                    <div class="font-bold text-blue-700 dark:text-blue-400 leading-tight">{{ $lead->name }}</div>
                                    <div class="text-[10px] text-slate-500 dark:text-slate-400">{{ $lead->phone }}</div>
                                </div>
                                <div class="flex items-center gap-2">
                ...
                <div>
                    ...
                    <div class="space-y-2 max-h-56 overflow-y-auto">
                        @forelse($metrics['due_members_list'] as $dm)
                            <div class="p-2.5 rounded-xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800/80 flex items-center justify-between text-xs">
                                <div>
                                    <div class="font-bold text-slate-900 dark:text-white leading-tight">{{ $dm->member?->full_name }}</div>
                                    <div class="text-[10px] text-slate-500 dark:text-slate-400">{{ $dm->plan?->name ?? 'Membership' }}</div>
                                </div>
                                <div class="flex items-center gap-2">
                                    <div class="text-right">
                ...
                <div>
                    ...
                    <div class="space-y-2 max-h-56 overflow-y-auto">
                        @forelse($metrics['recent_renewals_list'] as $rn)
                            <div class="p-2.5 rounded-xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800/80 flex items-center justify-between text-xs">
                                <div>
                                    <div class="font-bold text-indigo-700 dark:text-indigo-400 leading-tight">{{ $rn->member?->full_name }}</div>
                                    <div class="text-[10px] text-slate-500 dark:text-slate-400">{{ $rn->plan?->name ?? 'Standard Plan' }}</div>
                                </div>
                                <div class="text-right">
                ...
                <div>
                    ...
                    <div class="space-y-2 max-h-56 overflow-y-auto">
                        @forelse($metrics['recent_attendance'] as $att)
                            <div class="p-2.5 rounded-xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800/80 flex items-center justify-between text-xs">
                                <div class="flex items-center gap-2.5">
                                    <div class="w-7 h-7 rounded-full bg-teal-100 dark:bg-teal-500/20 text-teal-700 dark:text-teal-400 flex items-center justify-center font-bold text-[10px]">
                                        {{ substr($att->member?->first_name ?? 'M', 0, 1) }}
                                    </div>
                                    <div>
                                        <div class="font-bold text-slate-900 dark:text-white leading-tight">{{ $att->member?->full_name }}</div>
                                        <div class="text-[10px] text-slate-500 dark:text-slate-400">{{ \Carbon\Carbon::parse($att->check_in)->format('h:i A') }}</div>
                                    </div>
                                </div>
                                <div>
                                    @if($att->check_out)
        ...
        <div x-show="showExpiringModal" 
            ...
            <div @click.away="showExpiringModal = false" 
                ...
                <!-- Modal Header -->
                <div class="p-5 bg-amber-50 dark:bg-amber-500/10 border-b border-amber-200 dark:border-amber-500/20 flex items-start justify-between">
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 rounded-2xl bg-amber-100 dark:bg-amber-500/20 text-amber-800 dark:text-amber-400 flex items-center justify-center">
                        ...
                        <div>
                            <h3 class="text-base font-black text-amber-800 dark:text-amber-400">Expiring Soon</h3>
                            <p class="text-xs text-amber-700 dark:text-amber-300/80 font-medium">Members expiring within 7 days</p>
                        </div>
                    </div>
                    <button @click="showExpiringModal = false" class="p-1.5 text-slate-400 hover:text-slate-800 dark:hover:text-white rounded-lg cursor-pointer">✕</button>
                </div>
                ...
                <!-- Modal Footer -->
                <div class="p-4 bg-slate-50 dark:bg-slate-950 border-t border-slate-200 dark:border-slate-800 flex items-center justify-between text-xs">
                    <span class="text-slate-600 dark:text-slate-400 font-medium">{{ count($metrics['expiring_members_list']) }} members expiring within 7 days</span>
                    <a href="{{ route('app.members.index', ['filter' => 'expiring']) }}" class="text-amber-700 dark:text-amber-400 font-bold hover:underline flex items-center gap-1">
                        <span>↗ View All</span>
                    </a>
                </div>
            </div>
        ...
        <div x-show="showBirthdayModal" 
            ...
            <div @click.away="showBirthdayModal = false" 
                 class="w-full max-w-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl overflow-hidden shadow-2xl space-y-0 text-slate-900 dark:text-slate-100">
                <!-- Modal Header -->
                <div class="p-5 bg-pink-50 dark:bg-pink-500/10 border-b border-pink-200 dark:border-pink-500/20 flex items-start justify-between">
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 rounded-2xl bg-pink-100 dark:bg-pink-500/20 text-pink-700 dark:text-pink-400 flex items-center justify-center text-xl">
                            🎂
                        </div>
                        <div>
                            <h3 class="text-base font-black text-pink-700 dark:text-pink-400">Birthdays Today</h3>
                            <p class="text-xs text-pink-600 dark:text-pink-300/80 font-medium">Celebrate and send wishes to members born today</p>
                        </div>
                    </div>
                    <button @click="showBirthdayModal = false" class="p-1.5 text-slate-400 hover:text-slate-800 dark:hover:text-white rounded-lg cursor-pointer">✕</button>
                </div>
                ...
                <!-- Modal Footer -->
                <div class="p-4 bg-slate-50 dark:bg-slate-950 border-t border-slate-200 dark:border-slate-800 flex items-center justify-between text-xs">
                    <span class="text-slate-600 dark:text-slate-400 font-medium">{{ count($metrics['birthday_members_list'] ?? []) }} birthdays today</span>
                    <a href="{{ route('app.members.index', ['filter' => 'birthday']) }}" class="text-pink-700 dark:text-pink-400 font-bold hover:underline flex items-center gap-1">
