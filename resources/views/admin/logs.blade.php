    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <form action="{{ route('admin.logs') }}" method="GET" class="flex flex-wrap items-center gap-3 flex-grow max-w-xl">
                <input type="text" name="action" value="{{ request('action') }}" placeholder="Filter by action (e.g. member_created, subscription_activated)..." class="px-4 py-2.5 rounded-xl bg-slate-900 border border-slate-800 text-white placeholder-slate-500 text-xs focus:border-red-500 focus:outline-none flex-grow">
                <select name="tenant_id" class="px-3 py-2.5 rounded-xl bg-slate-900 border border-slate-800 text-white text-xs focus:border-red-500 focus:outline-none">
                    ...
                    <tbody class="divide-y divide-slate-800/60 text-slate-300">
                        @forelse($logs as $log)
                            <tr class="hover:bg-slate-800/30 transition-colors">
                                ...
                                <td class="py-3 px-4 font-mono font-bold text-red-400 text-[11px]">
                                    {{ $log->action }}
                                </td>
                                <td class="py-3 px-4 text-slate-300 text-[11px] max-w-sm truncate">
                                    {{ $log->description }}
                                </td>
