            ...
            <div class="flex items-center gap-1.5 flex-wrap">
                ...
                <a href="{{ route('app.staff.index') }}" 
                   class="px-3.5 py-1.5 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 bg-indigo-600 text-white shadow-sm">
                    ...
                    <span class="px-1.5 py-0.5 rounded-full text-[10px] bg-white/20 dark:bg-slate-950/60 font-black">{{ $totalStaff }}</span>
                </a>

                <!-- Roles & Permissions Tab -->
                <a href="{{ route('app.roles.index') }}" 
                   class="px-3.5 py-1.5 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800 border border-slate-200 dark:border-slate-800 shadow-sm">
                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/></svg>
                    <span>Roles & Permissions</span>
                </a>

                @if(isset($staffQuota))
                <span class="px-2.5 py-1 rounded-xl bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-200 dark:border-indigo-500/20 text-indigo-700 dark:text-indigo-300 text-xs font-bold font-mono flex items-center gap-1.5">
                    <span class="w-1.5 h-1.5 rounded-full {{ $staffQuota['allowed'] ? 'bg-emerald-500 dark:bg-emerald-400' : 'bg-amber-500 dark:bg-amber-400' }}"></span>
        ...
        <form method="GET" action="{{ route('app.staff.index') }}" class="p-3.5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm flex flex-wrap items-center gap-2.5">
            ...
            <div class="flex-1 min-w-[240px] relative">
                ...
                <input type="text" 
                       ...
                       class="w-full pl-10 pr-4 py-2 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-800 text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 text-xs focus:border-indigo-500 focus:outline-none transition-colors">
            </div>
            ...
            <div>
                <select name="role" 
                    ...
                    @if(isset($roles))
                        @foreach($roles->where('is_system', false) as $cr)
                            <option value="{{ $cr->name }}" {{ request('role') === $cr->name ? 'selected' : '' }}>{{ $cr->display_name ?? ucfirst($cr->name) }} (Custom)</option>
                        @endforeach
                    @endif
                </select>
            </div>
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-800/60 text-slate-700 dark:text-slate-300">
                        @forelse($staffMembers as $u)
                            ...
                            <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                                ...
                                <td class="py-3.5 px-4">
                                    @if(!empty($empId))
                                        <span class="px-2 py-0.5 rounded bg-slate-100 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 font-mono text-[11px] font-semibold text-slate-700 dark:text-slate-300">
                                            {{ $empId }}
                                        </span>
                                    @else
                                ...
                                <!-- PHONE -->
                                <td class="py-3.5 px-4 text-slate-600 dark:text-slate-300 font-mono text-xs">
                                    {{ $u->phone ?: '—' }}
                                </td>

                                <!-- SALARY -->
                                <td class="py-3.5 px-4 font-mono text-xs text-slate-800 dark:text-slate-200">
                                    @if($salaryVal !== null)
                                ...
                                <td class="py-3.5 px-4">
                                    @if($canLogin)
                                        <span class="px-2 py-0.5 rounded-full bg-emerald-50 dark:bg-emerald-500/15 border border-emerald-200 dark:border-emerald-500/30 text-emerald-700 dark:text-emerald-400 font-bold text-[10px] inline-flex items-center gap-1">
                                            ✓ On
                                        </span>
                                    @else
                                        <span class="px-2 py-0.5 rounded-full bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400 font-bold text-[10px] inline-flex items-center gap-1">
                                            ✕ Off
                                        </span>
                                    @endif
                                </td>
                                ...
                                <td class="py-3.5 px-4">
                                    ...
                                    @else
                                        <span class="px-2.5 py-0.5 rounded-full bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 font-bold text-[10px]">
                                            Inactive
                                        </span>
                                    @endif
                                </td>
                                ...
                                <td class="py-3.5 px-4 text-right">
                                    <div class="flex items-center justify-end gap-1.5">
                                        ...
                                        <button type="button" 
                                                ...
                                                class="p-1.5 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white transition-colors cursor-pointer border border-slate-200 dark:border-slate-700">
                                        ...
                                        <button type="button" 
                                                ...
                                                class="p-1.5 rounded-xl bg-amber-50 dark:bg-slate-800 hover:bg-amber-100 dark:hover:bg-slate-700 text-amber-700 dark:text-amber-400 hover:text-amber-800 dark:hover:text-amber-300 transition-colors cursor-pointer border border-amber-200 dark:border-slate-700">
                                        ...
                                        @if($u->id !== auth()->id() && $u->role !== 'gym_owner')
                                            ...
                                            <form action="{{ route('app.staff.delete', $u->id) }}" method="POST" 
                                                ...
                                                <button type="submit" 
                                                        ...
                                                        class="p-1.5 rounded-xl bg-rose-50 dark:bg-slate-800 hover:bg-rose-100 dark:hover:bg-rose-900/50 text-rose-600 dark:text-rose-400 transition-colors cursor-pointer border border-rose-200 dark:border-slate-700">
                        ...
                        @empty
                            <tr>
                                <td colspan="9" class="py-12 text-center text-slate-500">
                                    ...
                                    <p class="text-xs text-slate-500 mt-1">Click "+ Add Staff" to add front desk, trainers, or managers.</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
        ...
        <div x-show="showAddStaffModal || showEditStaffModal" 
            ...
            <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl max-w-5xl w-full p-5 sm:p-7 shadow-2xl relative my-6 text-slate-900 dark:text-slate-200" 
                ...
                <form :action="showEditStaffModal ? ('{{ url('app/staff') }}/' + staffForm.id) : '{{ route('app.staff.store') }}'" 
                    ...
                    <div class="space-y-3">
                        <h4 class="text-[11px] font-extrabold text-slate-500 dark:text-slate-400 uppercase tracking-wider">BASIC INFO</h4>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-3.5">
                            ...
                            <div>
                                <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Phone <span class="text-rose-500">*</span></label>
                                <input type="text" 
                                       name="phone" 
                            ...
                            <div>
                                <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Email <span class="text-slate-400 text-[10px] font-normal">(Optional)</span></label>
                                <input type="email" 
                                       name="email" 
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-3.5 items-center pt-1">
                            <div>
                                <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Role <span class="text-rose-500">*</span></label>
                                <select name="role" 
                                        x-model="staffForm.role" 
                                        required 
                                        class="w-full px-3.5 py-2 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-800 text-slate-900 dark:text-white text-xs font-semibold focus:border-indigo-500 focus:outline-none cursor-pointer">
                                    ...
                                    @if(isset($roles) && $roles->where('is_system', false)->count() > 0)
                                        <optgroup label="Custom Roles">
                                            @foreach($roles->where('is_system', false) as $customRole)
                    ...
                    <div class="space-y-3 pt-3 border-t border-slate-200 dark:border-slate-800/80">
                        <h4 class="text-[11px] font-extrabold text-slate-500 dark:text-slate-400 uppercase tracking-wider">EMPLOYMENT</h4>
                        
                        <!-- Row 1: Employee ID, Device Emp No, Maid ID -->
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-3.5">
                            <div>
                                <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Employee ID</label>
                                <input type="text" 
                                       name="employee_id" 
                            ...
                            <div>
                                <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Device Emp No <span class="text-slate-400 text-[10px]">(eSSL)</span></label>
                                <input type="text" 
                                       name="device_emp_no" 
                            ...
                            <div>
                                <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Maid ID <span class="text-slate-400 text-[10px]">(Optional)</span></label>
                                <input type="text" 
                                       name="maid_id" 
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3.5">
                            <div>
                                <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Designation</label>
                                <input type="text" 
                                       name="designation" 
                            ...
                            <div>
                                <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Employee Category</label>
                                <select name="employee_category" 
                                        x-model="staffForm.employee_category" 
                                        class="w-full px-3.5 py-2 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-800 text-slate-900 dark:text-white text-xs focus:border-indigo-500 focus:outline-none cursor-pointer">
                            ...
                            <div>
                                <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Joining Date</label>
                                <input type="date" 
                                       name="joining_date" 
                                       x-model="staffForm.joining_date" 
                                       class="w-full px-3.5 py-2 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-800 text-slate-900 dark:text-white text-xs font-mono focus:border-indigo-500 focus:outline-none">
                            </div>
                        </div>
                    </div>
                    ...
                    <!-- 3. SALARY SECTION -->
                    <div class="space-y-3 pt-3 border-t border-slate-200 dark:border-slate-800/80">
                        <h4 class="text-[11px] font-extrabold text-slate-500 dark:text-slate-400 uppercase tracking-wider">SALARY</h4>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3.5">
                            <div>
                                <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Monthly Salary</label>
                                <input type="number" 
                                       step="0.01" 
                                       name="monthly_salary" 
                                       x-model="staffForm.monthly_salary" 
                                       placeholder="0.00" 
                                       class="w-full px-3.5 py-2 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-800 text-slate-900 dark:text-white font-mono text-xs focus:border-indigo-500 focus:outline-none">
                            </div>

                            <div>
                                <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Payout Type</label>
                    ...
                    <div class="space-y-3 pt-3 border-t border-slate-200 dark:border-slate-800/80">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-3.5">
                            ...
                            <div>
                                <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Anniversary</label>
                                <input type="date" 
                                       name="anniversary" 
                    ...
                    <!-- 5. BANKING SECTION -->
                    <div class="space-y-3 pt-3 border-t border-slate-200 dark:border-slate-800/80">
                        <h4 class="text-[11px] font-extrabold text-slate-500 dark:text-slate-400 uppercase tracking-wider">BANKING</h4>
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-3.5">
                            <div>
                                <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">PAN Card</label>
                                <input type="text" 
                                       name="pan_card" 
                                       x-model="staffForm.pan_card" 
                                       placeholder="ABCDE1234F" 
                                       class="w-full px-3.5 py-2 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-800 text-slate-900 dark:text-white text-xs font-mono uppercase focus:border-indigo-500 focus:outline-none">
                            </div>

                            <div>
                                <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Bank Account No.</label>
                                <input type="text" 
                            ...
                            <div>
                                <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Bank IFSC</label>
                                <input type="text" 
                                       name="bank_ifsc" 
                    ...
                    <!-- 6. BRANCHES SECTION -->
                    <div class="space-y-2 pt-3 border-t border-slate-200 dark:border-slate-800/80">
                        <h4 class="text-[11px] font-extrabold text-slate-500 dark:text-slate-400 uppercase tracking-wider">BRANCHES</h4>
                        <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Assign Branches</label>
                            ...
                            <template x-for="b in allBranches" :key="b.id">
                                <div @click="toggleBranch(b.id)" 
                                     ...
                                     :class="staffForm.selected_branches.includes(b.id) 
                                        ? 'bg-indigo-50 dark:bg-indigo-950/40 border-indigo-300 dark:border-indigo-500 text-indigo-700 dark:text-indigo-300 shadow-sm' 
                                        : 'bg-slate-50 dark:bg-slate-950 border-slate-200 dark:border-slate-800/80 text-slate-600 dark:text-slate-400 hover:border-slate-300 dark:hover:border-slate-700'">
                                    ...
                                    <div x-show="staffForm.selected_branches.includes(b.id)" class="text-indigo-600 dark:text-indigo-400 text-xs font-bold">
                                        ✓
                                    </div>
                    ...
                    <!-- 7. OTHER SECTION -->
                    <div class="space-y-3 pt-3 border-t border-slate-200 dark:border-slate-800/80">
                        <h4 class="text-[11px] font-extrabold text-slate-500 dark:text-slate-400 uppercase tracking-wider">OTHER</h4>
                        ...
                        <div>
                            ...
                            <textarea name="address" 
                                      ...
                                      class="w-full px-3.5 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-800 text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-600 text-xs focus:border-indigo-500 focus:outline-none transition-colors"></textarea>
                        </div>
                    ...
                    <!-- 8. FOOTER BUTTONS -->
                    <div class="flex items-center justify-start gap-3 pt-4 border-t border-slate-200 dark:border-slate-800">
                        <button type="button" 
                                @click="showAddStaffModal = false; showEditStaffModal = false" 
                                class="px-4 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-transparent dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 font-bold text-xs transition-colors cursor-pointer border border-slate-200 dark:border-transparent">
                            Cancel
                        </button>
                        ...
                        <button type="submit" 
                                ...
                                class="px-5 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-xs flex items-center gap-1.5 shadow-lg shadow-indigo-600/30 transition-all cursor-pointer">
        ...
        <div x-show="showPasswordModal" 
            ...
            <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl max-w-md w-full p-6 shadow-2xl relative text-slate-900 dark:text-slate-200" 
                ...
                <button type="button" 
                        ...
                        class="absolute top-4 right-4 p-2 rounded-xl text-slate-400 hover:text-slate-600 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors cursor-pointer">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                </button>

                <div class="border-b border-slate-200 dark:border-slate-800 pb-3 mb-4">
                    <h3 class="text-base font-extrabold text-slate-900 dark:text-white flex items-center gap-2">
                        <svg class="w-5 h-5 text-amber-500 dark:text-amber-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z"/></svg>
                        <span>Reset Password</span>
                    </h3>
                    <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">Set a new login password for <span class="font-bold text-slate-900 dark:text-white" x-text="passwordStaffName"></span></p>
                </div>

                <form :action="'{{ url('app/staff') }}/' + passwordStaffId + '/reset-password'" method="POST" class="space-y-4">
                    @csrf
                    <div>
                        <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">New Password <span class="text-rose-500">*</span></label>
                        <input type="password" 
                               name="password" 
                               required 
                               minlength="6" 
                               placeholder="Minimum 6 characters" 
                               class="w-full px-3.5 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-800 text-slate-900 dark:text-white text-xs focus:border-indigo-500 focus:outline-none transition-colors">
                    </div>

                    <div class="flex items-center justify-end gap-2.5 pt-2">
                        <button type="button" 
                                @click="showPasswordModal = false" 
                                class="px-4 py-2 rounded-xl bg-slate-100 dark:bg-transparent hover:bg-slate-200 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 font-bold text-xs transition-colors cursor-pointer border border-slate-200 dark:border-transparent">
                            Cancel
                        </button>
                        <button type="submit" 
                                class="px-5 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-xs flex items-center gap-1.5 shadow-lg shadow-indigo-600/30 transition-all cursor-pointer">
