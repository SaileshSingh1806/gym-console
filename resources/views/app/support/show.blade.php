<x-app-layout header="Ticket #{{ $ticket->ticket_number }}">
    <div x-data="{ showCloseModal: false }" class="w-full space-y-4 sm:space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-200 dark:border-slate-800 pb-3">
            <div class="flex items-start sm:items-center gap-2 min-w-0">
                <a href="{{ route('app.support.index') }}" 
                   class="p-2 rounded-xl bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white border border-slate-200 dark:border-slate-800 transition-colors shadow-sm shrink-0">
                ...
                <div class="min-w-0">
                    <div class="flex items-center gap-2 flex-wrap">
                        ...
                        <h2 class="text-base font-black text-slate-900 dark:text-white tracking-tight break-words">{{ $ticket->subject }}</h2>
                    </div>
            ...
            <div class="flex items-center gap-2 shrink-0">
                @if(!in_array($ticket->status, ['resolved', 'closed']))
                    ...
                    <button type="button" @click="showCloseModal = true" class="w-full sm:w-auto px-3.5 py-2 rounded-xl bg-white hover:bg-slate-50 text-slate-700 border border-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 dark:text-slate-300 dark:hover:text-white text-xs font-bold dark:border-slate-700/60 transition-colors cursor-pointer flex items-center justify-center gap-1.5 shadow-sm">
                ...
                @else
                    <span class="px-3 py-1.5 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 text-xs font-bold border border-slate-200 dark:border-slate-700">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-4 sm:gap-6">
            ...
            <div class="lg:col-span-3 space-y-4">
                ...
                <div class="space-y-4">
                    ...
                    <div class="p-4 sm:p-5 rounded-2xl border {{ $reply->is_admin_reply ? 'bg-indigo-50/50 dark:bg-indigo-950/20 border-indigo-200 dark:border-indigo-500/30 shadow-sm' : 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 shadow-sm' }} space-y-3 overflow-hidden">
                        <!-- Reply Header -->
                        <div class="flex items-center justify-between gap-3 border-b {{ $reply->is_admin_reply ? 'border-indigo-100 dark:border-indigo-500/20' : 'border-slate-100 dark:border-slate-800' }} pb-2.5">
                            <div class="flex items-center gap-2.5 min-w-0">
                                <div class="w-8 h-8 rounded-xl {{ $reply->is_admin_reply ? 'bg-gradient-to-tr from-indigo-500 to-purple-600 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300' }} flex items-center justify-center font-bold text-xs shadow-sm shrink-0">
                                    {{ substr($reply->user->name ?? ($reply->is_admin_reply ? 'Admin' : 'User'), 0, 1) }}
                                </div>
                                <div class="min-w-0">
                                    <div class="flex items-center gap-2 flex-wrap">
                                        <span class="font-bold text-slate-900 dark:text-white text-xs truncate">{{ $reply->user->name ?? ($reply->is_admin_reply ? 'Support Team' : 'User') }}</span>
                ...
                <div class="p-4 sm:p-5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm space-y-3">
                    ...
                    <form action="{{ route('app.support.reply', $ticket->id) }}" method="POST" enctype="multipart/form-data" class="space-y-3 text-xs">
                        ...
                        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-1">
                            ...
                            <button type="submit" class="w-full sm:w-auto px-5 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-xs shadow-lg shadow-indigo-600/20 transition-all flex items-center justify-center gap-1.5 cursor-pointer">
        ...
        <div x-show="showCloseModal" x-cloak 
            ...
            <div @click.away="showCloseModal = false"
                 class="w-full max-w-md max-h-[90vh] overflow-y-auto bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl sm:rounded-3xl shadow-2xl p-4 sm:p-6 space-y-4 sm:space-y-5"
                 x-transition:enter="transition ease-out duration-200"
                 x-transition:enter-start="opacity-0 scale-95"
                 x-transition:enter-end="opacity-100 scale-100">
                
                <div class="flex items-start gap-3 sm:gap-4">
                    <div class="w-10 h-10 sm:w-12 sm:h-12 rounded-2xl bg-emerald-500/10 dark:bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20 dark:border-emerald-500/30 flex items-center justify-center shrink-0 shadow-inner">
