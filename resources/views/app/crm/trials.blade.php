    ...
    <div class="space-y-6" x-data="{
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-3.5">
            ...
            <div class="p-4 rounded-2xl bg-slate-900 border border-slate-800 flex items-center justify-between shadow-sm">
                <div class="space-y-1">
                    <span class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Today's Trials</span>
                    <div class="text-2xl font-black text-white tracking-tight">{{ $todayTrialsCount }}</div>
                </div>
            ...
            <div class="p-4 rounded-2xl bg-slate-900 border border-slate-800 flex items-center justify-between shadow-sm">
                <div class="space-y-1">
                    <span class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Upcoming</span>
                    <div class="text-2xl font-black text-purple-400 tracking-tight">{{ $upcomingTrialsCount }}</div>
                </div>
            ...
            <div class="p-4 rounded-2xl bg-slate-900 border border-slate-800 flex items-center justify-between shadow-sm">
                <div class="space-y-1">
                    <span class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Completed ({{ date('M', mktime(0, 0, 0, $month, 1)) }})</span>
                    <div class="text-2xl font-black text-emerald-400 tracking-tight">{{ $completedThisMonth }}</div>
                </div>
            ...
            <div class="p-4 rounded-2xl bg-slate-900 border border-slate-800 flex items-center justify-between shadow-sm">
                <div class="space-y-1">
                    <span class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Cancelled ({{ date('M', mktime(0, 0, 0, $month, 1)) }})</span>
                    <div class="text-2xl font-black text-rose-400 tracking-tight">{{ $cancelledThisMonth }}</div>
                </div>
        ...
        <div class="rounded-3xl bg-slate-900 border border-slate-800 overflow-hidden shadow-xl">
            ...
            <div class="p-4 border-b border-slate-800 flex items-center justify-between">
                <a href="{{ route('app.crm.trials', ['month' => $prevMonthDate->month, 'year' => $prevMonthDate->year]) }}" class="p-2 rounded-xl bg-slate-950 text-slate-400 hover:text-white border border-slate-800 hover:border-slate-700 transition-all">
                ...
                <a href="{{ route('app.crm.trials', ['month' => $nextMonthDate->month, 'year' => $nextMonthDate->year]) }}" class="p-2 rounded-xl bg-slate-950 text-slate-400 hover:text-white border border-slate-800 hover:border-slate-700 transition-all">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M9 5l7 7-7 7"/></svg>
                </a>
            </div>

            <!-- Weekday Column Headers (Mon - Sun) -->
            <div class="grid grid-cols-7 border-b border-slate-800/80 bg-slate-950/40 text-center text-xs font-bold text-slate-400 py-2.5">
                <div>Mon</div>
                <div>Tue</div>
                <div>Wed</div>
                <div>Thu</div>
            ...
            <!-- Calendar Days Grid -->
            <div class="grid grid-cols-7 divide-x divide-y divide-slate-800/60 bg-slate-950/20">
                <!-- Previous month trailing days -->
                @for($i = $startDayOfWeek - 1; $i >= 1; $i--)
                    @php $prevDayNum = $prevMonthDays - $i + 1; @endphp
                    <div class="min-h-[100px] p-2 bg-slate-950/30 text-slate-600 text-xs">
                ...
                @for($d = 1; $d <= $daysInMonth; $d++)
                    ...
                    @endphp
                    <div class="min-h-[100px] p-2 {{ $isToday ? 'bg-indigo-950/20 ring-1 ring-inset ring-indigo-500/30' : 'hover:bg-slate-800/20' }} transition-colors relative flex flex-col justify-between">
                        <div class="flex items-center justify-between mb-1">
                            <span class="text-xs font-bold {{ $isToday ? 'text-indigo-400 px-1.5 py-0.5 rounded-md bg-indigo-500/20' : 'text-slate-300' }}">{{ $d }}</span>
                        </div>

                        <!-- Scheduled Demo Event Chips -->
                        <div class="space-y-1 overflow-y-auto max-h-20">
                            @foreach($dayTrials as $tr)
                                <button type="button" @click="viewTrial({{ Js::from($tr) }})" class="w-full text-left p-1 rounded-md bg-indigo-950/80 hover:bg-indigo-900/90 text-indigo-300 border border-indigo-500/30 text-[10px] font-semibold truncate transition-colors flex items-center gap-1 cursor-pointer" title="{{ $tr->trial_time }} {{ $tr->lead?->name }}">
                                    <span class="w-1.5 h-1.5 rounded-full bg-indigo-400 shrink-0"></span>
                                    <span class="font-mono text-[9px] text-indigo-200">{{ $tr->trial_time ? substr($tr->trial_time, 0, 5) : '10:00' }}</span>
        ...
        <div class="p-5 rounded-3xl bg-slate-900 border border-slate-800 space-y-4 shadow-md">
            ...
            <div class="divide-y divide-slate-800/80">
                @forelse($upcomingList as $trial)
                    <div class="py-3 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs hover:bg-slate-800/30 px-2 rounded-xl transition-colors">
                        <div class="flex items-center gap-3">
                            <div class="w-10 h-10 rounded-xl bg-purple-500/10 text-purple-400 border border-purple-500/20 flex flex-col items-center justify-center shrink-0">
                                <span class="text-xs font-black leading-none">{{ $trial->trial_date ? $trial->trial_date->format('d') : '12' }}</span>
                                <span class="text-[8.5px] uppercase font-bold text-purple-300 leading-none mt-0.5">{{ $trial->trial_date ? $trial->trial_date->format('M') : 'Sep' }}</span>
                            </div>

                            <div class="min-w-0">
                                <h4 class="font-bold text-white truncate text-sm">{{ $trial->lead?->name ?? 'Guest Prospect' }}</h4>
                                <div class="text-slate-400 mt-0.5 flex flex-wrap items-center gap-2 text-[11px]">
                                    <span class="font-mono text-purple-300 font-bold">{{ $trial->trial_time ?? '10:00 AM' }}</span>
                                    <span>•</span>
                                    <span>{{ $trial->lead?->phone }}</span>
                                    @if($trial->assignedTo)
                                        <span>•</span>
                                        <span>Coach: <strong class="text-slate-200">{{ $trial->assignedTo->name }}</strong></span>
                                    @endif
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center gap-2 shrink-0">
                            <!-- Status Badge -->
                            <span class="px-2.5 py-0.5 rounded-lg text-[10px] font-extrabold uppercase {{ $trial->status === 'completed' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : ($trial->status === 'cancelled' ? 'bg-rose-500/10 text-rose-400 border border-rose-500/20' : 'bg-purple-500/10 text-purple-400 border border-purple-500/20') }}">
                                {{ $trial->status }}
                            </span>
                            ...
                            @if($trial->status === 'upcoming')
                                ...
                                <form action="{{ route('app.crm.trials.status', $trial->id) }}" method="POST" class="inline-block">
                                    ...
                                    <button type="submit" class="px-2.5 py-1 rounded-lg bg-slate-800 hover:bg-rose-500/20 text-slate-400 hover:text-rose-400 font-bold text-[10px] transition-all cursor-pointer">
                                        Cancel
                                    </button>
                                </form>
                            @endif
                            ...
                            <!-- Delete -->
                            <form action="{{ route('app.crm.trials.delete', $trial->id) }}" method="POST" onsubmit="return confirm('Remove this trial booking?')" class="inline-block">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="p-1 rounded-lg text-slate-500 hover:text-rose-400 transition-colors">
                ...
                @empty
                    <div class="py-12 text-center text-slate-500 text-xs space-y-2">
                        <div class="w-12 h-12 rounded-2xl bg-slate-950 flex items-center justify-center mx-auto text-slate-600 border border-slate-800">
                        ...
                        <p class="font-medium text-slate-400">No upcoming demos scheduled.</p>
            ...
            <div class="bg-slate-900 border border-slate-800 rounded-3xl max-w-lg w-full p-6 shadow-2xl space-y-4" @click.away="showTrialModal = false">
                ...
                <form action="{{ route('app.crm.trials.store') }}" method="POST" class="space-y-3.5">
                    ...
                    <div>
                        ...
                        <textarea name="notes" rows="2" placeholder="e.g. 1-day complimentary guest workout pass" class="w-full px-3.5 py-2 rounded-xl bg-slate-950 border border-slate-800 text-white text-xs focus:border-indigo-500 focus:outline-none"></textarea>
                    </div>
        ...
        <!-- ==================== MODAL: TRIAL DETAILS ==================== -->
        <div x-show="showDetailsModal" class="fixed inset-0 z-50 overflow-y-auto bg-black/80 backdrop-blur-sm flex items-center justify-center p-4" x-cloak>
            <div class="bg-slate-900 border border-slate-800 rounded-3xl max-w-md w-full p-6 shadow-2xl space-y-4" @click.away="showDetailsModal = false">
                <div class="flex items-center justify-between border-b border-slate-800 pb-3">
                    <h3 class="text-base font-black text-white tracking-tight">Demo Session Details</h3>
                    <button type="button" @click="showDetailsModal = false" class="text-slate-400 hover:text-white text-xs font-bold p-1">✕</button>
                </div>

                <template x-if="selectedTrial">
                    <div class="space-y-3 text-xs">
                        <div class="p-3 rounded-2xl bg-slate-950 border border-slate-800 space-y-1.5">
                            <div class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Prospect Info</div>
                            <div class="text-sm font-black text-white" x-text="selectedTrial.lead?.name || 'Guest Lead'"></div>
                            <div class="text-slate-400" x-text="selectedTrial.lead?.phone || 'No phone'"></div>
                        </div>

                        <div class="grid grid-cols-2 gap-2 text-center">
                            <div class="p-2.5 rounded-xl bg-slate-950 border border-slate-800">
                                <div class="text-[9px] font-bold text-slate-400 uppercase">Date</div>
                                <div class="font-bold text-white text-xs mt-0.5" x-text="selectedTrial.trial_date ? selectedTrial.trial_date.substring(0, 10) : ''"></div>
                            </div>
                            <div class="p-2.5 rounded-xl bg-slate-950 border border-slate-800">
                                <div class="text-[9px] font-bold text-slate-400 uppercase">Time</div>
                                <div class="font-bold text-indigo-400 text-xs mt-0.5" x-text="selectedTrial.trial_time || '10:00 AM'"></div>
                            </div>
                        </div>
                        ...
                        <div class="p-3 rounded-2xl bg-slate-950 border border-slate-800 space-y-1">
                            ...
                            <div class="text-slate-300" x-text="selectedTrial.notes || 'Complimentary trial workout session'"></div>
                        </div>
                    </div>
                </template>

                <div class="flex items-center justify-end gap-2 pt-2 border-t border-slate-800">
                    <button type="button" @click="showDetailsModal = false" class="px-4 py-2 rounded-xl bg-slate-800 text-slate-300 hover:text-white text-xs font-bold">Close</button>
                </div>
            </div>
        </div>
    </div>
