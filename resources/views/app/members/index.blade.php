    ...
    <div x-data="{ 
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div class="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 flex items-center justify-between shadow-sm">
                <div>
                    <span class="text-xs text-slate-500 dark:text-slate-400 font-medium">Total Enrolled</span>
                    <p class="text-2xl font-black text-slate-900 dark:text-white mt-1">{{ $totalMembers }}</p>
                </div>
                <div class="w-11 h-11 rounded-2xl bg-amber-50 dark:bg-amber-500/10 text-amber-600 dark:text-amber-400 flex items-center justify-center border border-amber-200 dark:border-amber-500/20">
            ...
            <div class="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 flex items-center justify-between shadow-sm">
                <div>
                    <span class="text-xs text-slate-500 dark:text-slate-400 font-medium">Active Members</span>
                    <p class="text-2xl font-black text-emerald-600 dark:text-emerald-400 mt-1">{{ $activeCount }}</p>
                </div>
                <div class="w-11 h-11 rounded-2xl bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 flex items-center justify-center border border-emerald-200 dark:border-emerald-500/20">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"/></svg>
                </div>
            </div>

            <a href="{{ route('app.members.index', ['filter' => 'due']) }}" 
               class="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 flex items-center justify-between hover:border-amber-500/40 hover:bg-slate-50 dark:hover:bg-slate-800/60 transition-all group cursor-pointer shadow-sm">
                <div>
                    <span class="text-xs text-slate-500 dark:text-slate-400 font-medium">Remaining Fees Due</span>
                    <p class="text-2xl font-black text-amber-600 dark:text-amber-400 mt-1">{{ $pendingFeesCount }} <span class="text-xs font-normal text-slate-500 dark:text-slate-400">members</span></p>
                </div>
                <div class="w-11 h-11 rounded-2xl bg-amber-50 dark:bg-amber-500/10 text-amber-600 dark:text-amber-400 flex items-center justify-center border border-amber-200 dark:border-amber-500/20 group-hover:scale-105 transition-transform">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"/></svg>
                </div>
            </a>

            <a href="{{ route('app.members.index', ['filter' => 'expiring']) }}" 
               class="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 flex items-center justify-between hover:border-rose-500/40 hover:bg-slate-50 dark:hover:bg-slate-800/60 transition-all group cursor-pointer shadow-sm">
                <div>
                    <span class="text-xs text-slate-500 dark:text-slate-400 font-medium">Expiring Soon</span>
                    <p class="text-2xl font-black text-rose-600 dark:text-rose-400 mt-1">{{ $expiringSoonCount }} <span class="text-xs font-normal text-slate-500 dark:text-slate-400">within 7d</span></p>
                </div>
                <div class="w-11 h-11 rounded-2xl bg-rose-50 dark:bg-rose-500/10 text-rose-600 dark:text-rose-400 flex items-center justify-center border border-rose-200 dark:border-rose-500/20 group-hover:scale-105 transition-transform">
        ...
        <div class="flex items-center gap-2 overflow-x-auto pb-1 text-xs font-bold no-scrollbar">
            ...
            <a href="{{ route('app.members.index', ['status' => 'ACTIVE']) }}" 
               class="px-3.5 py-2 rounded-xl border transition-all flex items-center gap-1.5 {{ request('status') === 'ACTIVE' && !request('filter') ? 'bg-emerald-500 text-slate-950 border-emerald-400 font-extrabold shadow-md shadow-emerald-500/20' : 'bg-white dark:bg-slate-900 text-emerald-600 dark:text-emerald-400 border-slate-200 dark:border-slate-800 hover:bg-emerald-50 dark:hover:bg-emerald-500/10 shadow-sm' }}">
            ...
            <a href="{{ route('app.members.index', ['filter' => 'due']) }}" 
               class="px-3.5 py-2 rounded-xl border transition-all flex items-center gap-1.5 {{ request('filter') === 'due' ? 'bg-amber-500 text-slate-950 border-amber-400 font-extrabold shadow-md shadow-amber-500/20' : 'bg-white dark:bg-slate-900 text-amber-600 dark:text-amber-400 border-slate-200 dark:border-slate-800 hover:bg-amber-50 dark:hover:bg-amber-500/10 shadow-sm' }}">
            ...
            <a href="{{ route('app.members.index', ['status' => 'INACTIVE']) }}" 
               class="px-3.5 py-2 rounded-xl border transition-all flex items-center gap-1.5 {{ request('status') === 'INACTIVE' ? 'bg-slate-700 text-white border-slate-600 font-extrabold shadow-md' : 'bg-white dark:bg-slate-900 text-slate-600 dark:text-slate-400 border-slate-200 dark:border-slate-800 hover:text-slate-900 dark:hover:text-white hover:bg-slate-50 dark:hover:bg-slate-800 shadow-sm' }}">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <form action="{{ route('app.members.index') }}" method="GET" class="flex flex-wrap items-center gap-3 flex-grow max-w-2xl">
                ...
                @endif
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search by name, phone, email, or member code..." class="px-4 py-2.5 rounded-xl bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-800 text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 text-xs focus:border-amber-500 focus:outline-none flex-grow min-w-[220px] shadow-sm">
                
                <select name="status" class="px-3 py-2.5 rounded-xl bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-800 text-slate-900 dark:text-white text-xs focus:border-amber-500 focus:outline-none shadow-sm">
                ...
                <button type="submit" class="px-4 py-2.5 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 border border-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 dark:text-slate-300 dark:border-slate-700 text-xs font-semibold flex items-center gap-1.5 transition-colors cursor-pointer shadow-sm">
                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
                    Search
                </button>
                @if(request('status') || request('filter') || request('search'))
                    <a href="{{ route('app.members.index') }}" class="text-xs text-rose-500 dark:text-rose-400 hover:underline">Clear Filters</a>
                @endif
            </form>
                    ...
                    @forelse($members as $member)
                        ...
                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                            ...
                            <td class="py-3 px-3 text-slate-600 dark:text-slate-400">
                                <span class="px-2 py-0.5 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 font-medium text-[10px] border border-slate-200 dark:border-slate-700/40 inline-block">
                            ...
                            <td class="py-3 px-3.5">
                                @if($membership && $membership->plan)
                                    <div class="font-bold text-slate-900 dark:text-white text-xs leading-tight">{{ $membership->plan->name }}</div>
                                    <div class="text-[10px] text-slate-500 dark:text-slate-400 leading-tight mt-0.5">
                                        ...
                                        @if($membership->isExpired() || $daysLeft < 0)
                                            <span class="text-rose-600 dark:text-rose-400 font-bold ml-0.5">(Expired)</span>
                                        @elseif($daysLeft == 0)
                                            <span class="text-amber-600 dark:text-amber-400 font-bold ml-0.5">(Today)</span>
                                        @else
                                            <span class="text-emerald-600 dark:text-emerald-400 font-semibold ml-0.5">({{ $daysLeft }}d left)</span>
                                        @endif
                                    </div>
                                @else
                                    <span class="text-amber-600 dark:text-amber-400/80 text-[10px] font-semibold italic">No Active Plan</span>
                                @endif
                            </td>
                            ...
                            <td class="py-3 px-3.5">
                                @if($membership)
                                    <div>
                                        @if($remainingAmt > 0)
                                            <span class="px-2 py-0.5 rounded-lg bg-amber-50 dark:bg-amber-500/15 border border-amber-200 dark:border-amber-500/30 text-amber-700 dark:text-amber-400 font-bold text-[11px] inline-flex items-center gap-1 shadow-sm leading-tight">
                                        ...
                                        @else
                                            <span class="px-2 py-0.5 rounded-lg bg-emerald-50 dark:bg-emerald-500/15 border border-emerald-200 dark:border-emerald-500/30 text-emerald-700 dark:text-emerald-400 font-bold text-[11px] inline-flex items-center gap-1 shadow-sm leading-tight">
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="py-12 text-center text-slate-500 dark:text-slate-400">
                                ...
                                <button @click="showAddModal = true" class="mt-3 px-4 py-2 rounded-xl bg-amber-500 text-slate-950 font-bold text-xs shadow-md shadow-amber-500/20">Enroll First Member</button>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
        ...
        <div x-show="showAddModal" class="fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 dark:bg-black/80 backdrop-blur-sm flex items-center justify-center p-4" x-cloak>
            <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl max-w-2xl w-full p-6 shadow-2xl space-y-5 text-slate-900 dark:text-slate-200" @click.away="showAddModal = false">
        ...
        <div x-show="showEditModal" 
             ...
             x-cloak>
            <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl max-w-4xl w-full p-6 sm:p-8 shadow-2xl space-y-6 my-8 max-h-[92vh] overflow-y-auto text-slate-900 dark:text-slate-200" @click.away="showEditModal = false">
        ...
        <div x-show="showCollectFeeModal" class="fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 dark:bg-black/80 backdrop-blur-sm flex items-center justify-center p-4" x-cloak>
            <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl max-w-lg w-full p-6 shadow-2xl space-y-5 text-slate-900 dark:text-slate-200" @click.away="showCollectFeeModal = false">
                <div class="flex justify-between items-center border-b border-slate-200 dark:border-slate-800 pb-3">
                    <div>
