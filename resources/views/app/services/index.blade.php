<x-app-layout header="Gym Services & Amenities">
    <div class="space-y-6" x-data="{
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div class="flex items-center gap-3">
                <div class="w-10 h-10 rounded-2xl bg-teal-500/10 dark:bg-teal-500/20 text-teal-600 dark:text-teal-400 border border-teal-500/20 dark:border-teal-500/30 flex items-center justify-center shadow-inner">
                ...
                <div>
                    ...
                    <p class="text-xs text-slate-500 dark:text-slate-400">Manage spa, sauna, massages, locker rentals, and add-on gym facilities.</p>
                </div>
            </div>

            <div class="flex items-center gap-3">
                <button type="button" @click="showBookingModal = true" class="px-4 py-2.5 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 hover:text-slate-900 dark:hover:text-white text-xs font-bold border border-slate-200 dark:border-slate-700 transition-all flex items-center gap-2">
        ...
        <div x-show="activeTab === 'services'" class="space-y-6">
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                @forelse($services as $svc)
                    <div class="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-700 transition-all flex flex-col justify-between shadow-sm group">
                        <div>
                            <!-- Card Header -->
                            <div class="flex items-start justify-between gap-2 mb-2">
                                <div class="w-9 h-9 rounded-xl bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 border border-indigo-500/20 flex items-center justify-center shrink-0">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z"/></svg>
                                </div>

                                <div>
                                    @if($svc->is_visible_in_portal)
                                        <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[9px] font-bold bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20">
                                            <span class="w-1.5 h-1.5 rounded-full bg-emerald-500 dark:bg-emerald-400"></span>
                                            <span>Visible</span>
                                        </span>
                                    @else
                                        <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[9px] font-bold bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-slate-700">
                            ...
                            <h3 class="text-sm font-bold text-slate-900 dark:text-white tracking-tight leading-tight">{{ $svc->name }}</h3>
                            ...
                            <p class="text-slate-500 dark:text-slate-400 text-[11px] leading-relaxed mt-2 line-clamp-2 min-h-[28px]">
                                {{ $svc->description ?? 'Premium gym amenity service available for members.' }}
                            </p>

                            <!-- Metadata Box -->
                            <div class="p-3 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800/80 my-3 space-y-1.5 text-[11px] text-slate-700 dark:text-slate-300">
                                <div class="flex justify-between items-center">
                                    <span class="text-slate-500 dark:text-slate-400">Duration:</span>
                                    <span class="font-semibold text-slate-900 dark:text-white">{{ $svc->duration_minutes }} mins</span>
                                </div>

                                <div class="flex justify-between items-center">
                                    <span class="text-slate-500 dark:text-slate-400">Availability:</span>
                                    <span class="font-semibold text-slate-800 dark:text-slate-200">{{ $svc->timeslot_availability ?? 'By Appointment' }}</span>
                                </div>

                                @if($svc->is_locker_service)
                                    <div class="pt-1 border-t border-slate-200 dark:border-slate-800/60 flex items-center justify-between">
                                        <span class="text-slate-500 dark:text-slate-400">Service Type:</span>
                                        <span class="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[9px] font-bold bg-indigo-500/10 dark:bg-indigo-500/15 text-indigo-700 dark:text-indigo-300 border border-indigo-500/20 dark:border-indigo-500/30">
                                            <span>🔒 Locker</span>
                                        </span>
                                    </div>
                                @endif
                        ...
                        <!-- Card Action Buttons -->
                        <div class="pt-2.5 border-t border-slate-100 dark:border-slate-800 flex items-center gap-1.5">
                            <button type="button" @click="openEditService({{ json_encode($svc) }})"
                                    class="flex-1 py-1.5 px-2.5 rounded-lg bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 hover:text-slate-900 dark:hover:text-white text-xs font-semibold flex items-center justify-center gap-1 transition-colors">
                            ...
                            <form action="{{ route('app.services.toggle-visibility', $svc->id) }}" method="POST" class="inline">
                                @csrf
                                @if($svc->is_visible_in_portal)
                                    <button type="submit" class="py-1.5 px-2.5 rounded-lg bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-amber-600 dark:text-amber-400 hover:text-amber-700 dark:hover:text-amber-300 text-xs font-semibold border border-slate-200 dark:border-slate-700 transition-colors" title="Hide from Member Portal">
                                        <span>Hide</span>
                                    </button>
                                @else
                                    <button type="submit" class="py-1.5 px-2.5 rounded-lg bg-emerald-500/10 dark:bg-emerald-500/15 hover:bg-emerald-500/20 dark:hover:bg-emerald-500/25 text-emerald-600 dark:text-emerald-400 text-xs font-semibold border border-emerald-500/20 dark:border-emerald-500/30 transition-colors" title="Show in Member Portal">
                                        <span>Show</span>
                                    </button>
                                @endif
                            </form>
                            ...
                            <form action="{{ route('app.services.delete', $svc->id) }}" method="POST" 
                                ...
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="p-1.5 rounded-lg bg-rose-500/10 hover:bg-rose-500 hover:text-white text-rose-600 dark:text-rose-400 text-xs font-bold transition-colors cursor-pointer" title="Delete Service">
                ...
                @empty
                    <div class="col-span-full p-12 text-center bg-white dark:bg-slate-900 border border-dashed border-slate-200 dark:border-slate-800 rounded-3xl shadow-sm">
                        <div class="w-12 h-12 rounded-2xl bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 border border-indigo-500/20 flex items-center justify-center mx-auto mb-3">
                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z"/></svg>
                        </div>
                        <h3 class="text-sm font-bold text-slate-900 dark:text-white mb-1">No Services Added Yet</h3>
                        <p class="text-xs text-slate-500 dark:text-slate-400 max-w-sm mx-auto mb-4">Create your first gym service (Massages, Sauna, Locker rentals, etc.) to offer to members.</p>
        ...
        <div x-show="activeTab === 'requests'" class="space-y-4">
            <div class="rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 overflow-hidden shadow-sm">
                <div class="p-4 bg-slate-50 dark:bg-slate-950/60 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
                    <div>
                        <h3 class="text-xs font-bold text-slate-900 dark:text-white uppercase tracking-wider">Pending Member Booking Requests</h3>
                        <p class="text-[11px] text-slate-500 dark:text-slate-400">Review, approve, assign lockers, and confirm service reservations.</p>
                    </div>
                    <span class="text-xs px-2.5 py-1 rounded-full bg-amber-500/10 dark:bg-amber-500/20 text-amber-700 dark:text-amber-400 font-bold border border-amber-500/20 dark:border-amber-500/30">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-left text-xs">
                        ...
                        <tbody class="divide-y divide-slate-100 dark:divide-slate-800 text-slate-700 dark:text-slate-300">
                                ...
                                <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                                    <td class="py-3.5 px-4 font-bold text-slate-900 dark:text-white">
                                        <div class="flex items-center gap-2">
                                            <div class="w-6 h-6 rounded-full bg-slate-100 dark:bg-slate-800 text-indigo-600 dark:text-indigo-400 flex items-center justify-center font-bold text-[10px]">
                                                {{ substr($req->member->first_name, 0, 1) }}
                                            </div>
                                            <span>{{ $req->member->full_name }}</span>
                                            @if($req->member->member_code)
                                                <span class="text-[10px] text-slate-400 dark:text-slate-500 font-mono">- {{ $req->member->member_code }}</span>
                                            @endif
                                        </div>
                                    </td>
                                    <td class="py-3.5 px-4 font-semibold text-teal-600 dark:text-teal-400">
                                        {{ $req->service->name }}
                                    </td>
                                    <td class="py-3.5 px-4 text-slate-700 dark:text-slate-300">
                                        {{ $req->booking_date->format('d M Y') }}
                                        @if($req->booking_time)
                                            <span class="text-[10px] text-slate-400 dark:text-slate-500 block">{{ date('h:i A', strtotime($req->booking_time)) }}</span>
                                        @endif
                                    </td>
                                    <td class="py-3.5 px-4 font-bold text-slate-900 dark:text-white">
                                        ...
                                        @if($req->service->is_locker_service)
                                            <form action="{{ route('app.services.bookings.status', $req->id) }}" method="POST" class="flex items-center gap-2">
                                                ...
                                                <input type="text" name="locker_number" value="{{ $req->locker_number }}" placeholder="e.g. L-102"
                                                       class="w-24 px-2 py-1 rounded bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-800 text-xs text-slate-900 dark:text-white">
                                    ...
                                    <td class="py-3.5 px-4 text-right">
                                        <div class="flex items-center justify-end gap-2">
                                            ...
                                            <form action="{{ route('app.services.bookings.status', $req->id) }}" method="POST" class="inline">
                                                ...
                                                <button type="submit" class="px-3 py-1.5 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-rose-50 dark:hover:bg-rose-950/60 text-slate-600 dark:text-slate-400 hover:text-rose-600 dark:hover:text-rose-400 font-semibold text-xs border border-slate-200 dark:border-slate-700">
                                                    Reject
                                                </button>
                                            </form>
                                        </div>
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="p-10 text-center text-slate-400 dark:text-slate-500 text-xs">
        ...
        <div x-show="activeTab === 'bookings'" class="space-y-4">
            <div class="rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 overflow-hidden shadow-sm">
                <div class="p-4 bg-slate-50 dark:bg-slate-950/60 border-b border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div>
                        <h3 class="text-xs font-bold text-slate-900 dark:text-white uppercase tracking-wider">All Member Service Bookings</h3>
                        <p class="text-[11px] text-slate-500 dark:text-slate-400">Track active sessions, deduction history, and locker allocations.</p>
                    </div>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-left text-xs">
                        ...
                        <tbody class="divide-y divide-slate-100 dark:divide-slate-800 text-slate-700 dark:text-slate-300">
                                ...
                                <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                                    <td class="py-3.5 px-4 font-bold text-slate-900 dark:text-white">
                                        <div class="flex items-center gap-2">
                                            ...
                                            @if($bk->member->member_code)
                                                <span class="text-[11px] text-slate-400 dark:text-slate-500 font-mono">- {{ $bk->member->member_code }}</span>
                                            @endif
                                        </div>
                                    </td>
                                    ...
                                    <td class="py-3.5 px-4">
                                        @if($bk->service->is_session_countable)
                                            <span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-bold {{ $bk->sessions_left > 0 ? 'bg-teal-500/10 dark:bg-teal-500/20 text-teal-700 dark:text-teal-400 border border-teal-500/20 dark:border-teal-500/30' : 'bg-slate-100 dark:bg-slate-800 text-slate-500' }}">
                                                <span class="w-1.5 h-1.5 rounded-full {{ $bk->sessions_left > 0 ? 'bg-teal-500 dark:bg-teal-400' : 'bg-slate-400 dark:bg-slate-600' }}"></span>
                                                <span>{{ $bk->sessions_left }}/{{ $bk->total_sessions }} left</span>
                                            </span>
                                        @else
                                            <span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-teal-500/10 dark:bg-teal-500/20 text-teal-700 dark:text-teal-400 border border-teal-500/20 dark:border-teal-500/30">
                                                <span>{{ $bk->sessions_left }}/{{ $bk->total_sessions }} left</span>
                                            </span>
                                        @endif
                                    </td>
                                    ...
                                    <td class="py-3.5 px-4">
                                        @if($bk->locker_number)
                                            <span class="px-2 py-0.5 rounded bg-indigo-500/10 dark:bg-indigo-500/20 text-indigo-700 dark:text-indigo-300 font-mono font-bold text-[10px] border border-indigo-500/20 dark:border-indigo-500/30">
                                                {{ $bk->locker_number }}
                                            </span>
                                        @else
                                            <span class="text-slate-400 dark:text-slate-500">-</span>
                                    ...
                                    <td class="py-3.5 px-4">
                                        @if($bk->status === 'active')
                                            <span class="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-indigo-500/10 dark:bg-indigo-500/15 text-indigo-700 dark:text-indigo-400 border border-indigo-500/20 dark:border-indigo-500/30">
                                                Active
                                            </span>
                                        ...
                                        @elseif($bk->status === 'pending')
                                            <span class="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-amber-500/10 dark:bg-amber-500/20 text-amber-700 dark:text-amber-400 border border-amber-500/20 dark:border-amber-500/30">
                                                Pending
                                            </span>
                                        @else
                                            <span class="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-rose-500/10 dark:bg-rose-500/20 text-rose-700 dark:text-rose-400">
                                                {{ ucfirst($bk->status) }}
                                            </span>
                                        @endif
                                    </td>
                                    <td class="py-3.5 px-4 text-right">
                                        <div class="flex items-center justify-end gap-2">
                                            ...
                                            <form action="{{ route('app.services.bookings.delete', $bk->id) }}" method="POST" 
                                                ...
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="p-1 rounded-lg text-slate-400 hover:text-rose-600 dark:text-slate-500 dark:hover:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-950/40 transition-colors cursor-pointer" title="Delete Booking">
                            ...
                            @empty
                                <tr>
                                    <td colspan="8" class="p-12 text-center text-slate-400 dark:text-slate-500 text-xs">
                ...
                <form :action="serviceFormAction" method="POST">
                    ...
                    <div class="p-5 bg-gradient-to-r from-indigo-50 via-slate-50 to-white dark:from-indigo-950/60 dark:via-slate-900 dark:to-slate-900 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <h3 class="text-base font-black text-slate-900 dark:text-white" x-text="isEditingService ? 'Edit Service' : 'Add New Service'">Add New Service</h3>
                    ...
                    <div class="p-6 space-y-4 max-h-[75vh] overflow-y-auto">
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" name="name" x-model="editServiceData.name" required placeholder="e.g. Steam Bath, Sauna, Massage"
                                       class="w-full px-3.5 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-800 text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 text-xs focus:outline-none focus:border-indigo-500 transition-colors">
                            </div>

                            <div>
                                <label class="block text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider mb-1">Amount (₹) <span class="text-rose-500">*</span></label>
                                <input type="number" name="amount" x-model="editServiceData.amount" required placeholder="0" min="0" step="1"
                                       class="w-full px-3.5 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-800 text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 text-xs font-bold focus:outline-none focus:border-indigo-500 transition-colors">
                            </div>
                        </div>

                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider mb-1">Duration (Minutes)</label>
                                <input type="number" name="duration_minutes" x-model="editServiceData.duration_minutes" placeholder="60" min="0"
                                       class="w-full px-3.5 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-800 text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 text-xs focus:outline-none focus:border-indigo-500 transition-colors">
                                <p class="text-[10px] text-slate-500 dark:text-slate-400 mt-1">How long does this service typically take?</p>
                            </div>

                            <div>
                                <label class="block text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider mb-1">Timeslot / Availability</label>
                                <input type="text" name="timeslot_availability" x-model="editServiceData.timeslot_availability" placeholder="e.g. 9 AM - 6 PM, Morning Only, By Appointment"
                                       class="w-full px-3.5 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-800 text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 text-xs focus:outline-none focus:border-indigo-500 transition-colors">
                                <p class="text-[10px] text-slate-500 dark:text-slate-400 mt-1">When is this service available?</p>
                            </div>
                        </div>
                        ...
                        <div class="space-y-3 pt-2 border-t border-slate-200 dark:border-slate-800">
                            ...
                            <label class="flex items-start gap-3 cursor-pointer p-3 rounded-xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-700 transition-colors">
                                <input type="checkbox" name="is_visible_in_portal" value="1" x-model="editServiceData.is_visible_in_portal"
                                       class="mt-0.5 rounded border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-indigo-600 focus:ring-0">
                                <div>
                                    <span class="text-xs font-bold text-slate-900 dark:text-white block">Show in Member Panel</span>
                                    <span class="text-[11px] text-slate-500 dark:text-slate-400">When enabled, members can see and request this service in their portal.</span>
                                </div>
                            </label>
                            ...
                            <label class="flex items-start gap-3 cursor-pointer p-3 rounded-xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-700 transition-colors">
                                <input type="checkbox" name="is_locker_service" value="1" x-model="editServiceData.is_locker_service"
                                       class="mt-0.5 rounded border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-indigo-600 focus:ring-0">
                                <div>
                                    <span class="text-xs font-bold text-slate-900 dark:text-white block">Locker Service</span>
                                    <span class="text-[11px] text-slate-500 dark:text-slate-400">When enabled, admin must assign a locker number when approving bookings.</span>
                                </div>
                            </label>
                            ...
                            <label class="flex items-start gap-3 cursor-pointer p-3 rounded-xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-700 transition-colors">
                                <input type="checkbox" name="is_session_countable" value="1" x-model="editServiceData.is_session_countable"
                                       class="mt-0.5 rounded border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-indigo-600 focus:ring-0">
                                <div>
                                    <span class="text-xs font-bold text-slate-900 dark:text-white block">Session Countable Pack</span>
                                    <span class="text-[11px] text-slate-500 dark:text-slate-400">The price buys a fixed number of sessions. Each visit is deducted until the pack runs out.</span>
                                </div>
                            </label>

                            <div x-show="editServiceData.is_session_countable" class="pl-8 pt-1">
                                <label class="block text-[11px] font-bold text-indigo-600 dark:text-indigo-400 uppercase tracking-wider mb-1">Total Sessions Included in Pack</label>
                                <input type="number" name="session_count" x-model="editServiceData.session_count" min="1" max="500" placeholder="e.g. 2, 5, 10"
                                       class="w-40 px-3 py-1.5 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-800 text-slate-900 dark:text-white text-xs font-bold focus:outline-none focus:border-indigo-500">
                            </div>
                        </div>
                    </div>
                ...
                <form action="{{ route('app.services.bookings.store') }}" method="POST">
                    ...
                    <div class="p-5 bg-gradient-to-r from-indigo-50 via-slate-50 to-white dark:from-indigo-950/60 dark:via-slate-900 dark:to-slate-900 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <h3 class="text-base font-black text-slate-900 dark:text-white">Book Service for Member</h3>
                    ...
                    <div class="p-6 space-y-4 max-h-[75vh] overflow-y-auto">
                        ...
                        <div>
                            <label class="block text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider mb-1">Select Service <span class="text-rose-500">*</span></label>
                            <select name="gym_service_id" required @change="updateAmountFromService($event)" class="w-full px-3.5 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-800 text-slate-900 dark:text-white text-xs focus:outline-none focus:border-indigo-500">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider mb-1">Booking Date <span class="text-rose-500">*</span></label>
                                <input type="date" name="booking_date" required value="{{ date('Y-m-d') }}"
                                       class="w-full px-3.5 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-800 text-slate-900 dark:text-white text-xs focus:outline-none focus:border-indigo-500">
                            </div>

                            <div>
                                <label class="block text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider mb-1">Time (Optional)</label>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider mb-1">Amount Paid (₹)</label>
                                <input type="number" name="amount_paid" :value="selectedServiceAmount" min="0" step="1"
                                       class="w-full px-3.5 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-800 text-slate-900 dark:text-white text-xs font-bold focus:outline-none focus:border-indigo-500">
                            </div>

                            <div>
                                <label class="block text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider mb-1">Locker Number (If applicable)</label>
                                <input type="text" name="locker_number" placeholder="e.g. L-104"
                                       class="w-full px-3.5 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-800 text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 text-xs focus:outline-none focus:border-indigo-500">
                            </div>
                        </div>

                        <div>
                            <label class="block text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider mb-1">Notes / Special Requests</label>
