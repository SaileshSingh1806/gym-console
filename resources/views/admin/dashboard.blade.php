<x-admin-layout header="SaaS Global Platform Metrics & Control">
    <div class="space-y-8" x-data="{ showNewGymModal: false, showNewPlanModal: false }">
        ...
        <div class="flex flex-wrap items-center justify-between gap-4 p-5 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm dark:shadow-xl dark:shadow-black/20 transition-colors">
            <div>
                <div class="flex items-center gap-2.5 mb-1.5">
                    <span class="w-2.5 h-2.5 rounded-full bg-emerald-500 animate-pulse"></span>
                    <h2 class="text-base font-bold text-slate-900 dark:text-white tracking-tight">Platform Command Center &bull; Administrator Controls</h2>
            ...
            <div class="flex items-center gap-3">
                <button @click="showNewGymModal = true" class="px-4 py-2.5 rounded-2xl bg-red-500 hover:bg-red-600 text-white font-bold text-xs flex items-center gap-2 shadow-lg shadow-red-500/20 transition-all cursor-pointer">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M12 4v16m8-8H4"/></svg>
                    <span>Register Gym</span>
                </button>
                <button @click="showNewPlanModal = true" class="px-4 py-2.5 rounded-2xl bg-slate-100 hover:bg-slate-200 text-slate-800 border border-slate-300 dark:bg-slate-800 dark:hover:bg-slate-700 dark:text-slate-200 dark:border-slate-700/60 font-bold text-xs flex items-center gap-2 transition-all cursor-pointer">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            ...
            <a href="{{ route('admin.gyms') }}" class="p-5 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 hover:border-indigo-500 shadow-sm hover:shadow-md dark:hover:shadow-indigo-500/5 transition-all group block">
                <div class="flex items-center justify-between mb-3">
                    <span class="text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Total Gym Businesses</span>
                    <div class="w-9 h-9 rounded-2xl bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 border border-indigo-200 dark:border-indigo-500/20 flex items-center justify-center text-base group-hover:scale-110 transition-transform">
                        🏢
                    </div>
                </div>
                <div class="text-3xl font-black text-slate-900 dark:text-white leading-none mb-2 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">
                    {{ $metrics['total_gyms'] }}
                </div>
            ...
            <a href="{{ route('admin.users') }}" class="p-5 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 hover:border-blue-500 shadow-sm hover:shadow-md dark:hover:shadow-blue-500/5 transition-all group block">
                <div class="flex items-center justify-between mb-3">
                    <span class="text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Total Platform Users</span>
                    <div class="w-9 h-9 rounded-2xl bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400 border border-blue-200 dark:border-blue-500/20 flex items-center justify-center text-base group-hover:scale-110 transition-transform">
                        👥
                    </div>
                </div>
                <div class="text-3xl font-black text-slate-900 dark:text-white leading-none mb-2 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">
                    {{ $totalUsers }}
                </div>
                <div class="text-xs text-slate-500 dark:text-slate-400 flex items-center gap-1.5 font-medium">
                    <span class="text-blue-600 dark:text-blue-300 font-semibold">{{ $totalStaffCount }} Staff & Owners</span>
                    <span class="text-slate-400">&bull;</span>
            ...
            <a href="{{ route('admin.tickets.index') }}" class="p-5 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 hover:border-rose-500 shadow-sm hover:shadow-md dark:hover:shadow-rose-500/5 transition-all group block">
                <div class="flex items-center justify-between mb-3">
                    <span class="text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Support Tickets</span>
                    <div class="w-9 h-9 rounded-2xl bg-rose-50 dark:bg-rose-500/10 text-rose-600 dark:text-rose-400 border border-rose-200 dark:border-rose-500/20 flex items-center justify-center text-base group-hover:scale-110 transition-transform">
                        🎫
                    </div>
                </div>
                <div class="text-3xl font-black {{ $openTicketsCount > 0 ? 'text-rose-600 dark:text-rose-400' : 'text-slate-900 dark:text-white' }} leading-none mb-2 group-hover:text-rose-500 dark:group-hover:text-rose-300 transition-colors">
                    {{ $openTicketsCount }} <span class="text-sm font-semibold text-slate-500 dark:text-slate-400 font-normal">Open</span>
                </div>
                <div class="text-xs text-slate-500 dark:text-slate-400 flex items-center gap-1.5 font-medium">
                    @if($urgentTicketsCount > 0)
                        ...
                        <span class="text-slate-400">&bull;</span>
                    @endif
                    <span>{{ $totalTicketsCount }} Total Logged</span>
                </div>
            </a>

            <!-- Card 4: Annual Platform Revenue (ARR) (Pure Yearly) -->
            <a href="{{ route('admin.subscriptions') }}" class="p-5 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 hover:border-emerald-500 shadow-sm hover:shadow-md dark:hover:shadow-emerald-500/5 transition-all group block">
                <div class="flex items-center justify-between mb-3">
                    <span class="text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Annual Revenue (ARR)</span>
                    <div class="w-9 h-9 rounded-2xl bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-500/20 flex items-center justify-center text-base group-hover:scale-110 transition-transform">
                        💰
                    </div>
                </div>
                <div class="text-3xl font-black text-emerald-600 dark:text-emerald-400 leading-none mb-2 group-hover:text-emerald-500 dark:group-hover:text-emerald-300 transition-colors">
                    ₹{{ number_format($metrics['arr'] ?? $metrics['total_revenue'], 2) }}
                </div>
                <div class="text-xs text-slate-500 dark:text-slate-400 flex items-center gap-1.5 font-medium">
                    <span>Invoiced: <strong class="text-slate-900 dark:text-white">₹{{ number_format($metrics['total_revenue'], 2) }}</strong></span>
                </div>
            </a>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 p-4 rounded-2xl bg-white dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800/80 shadow-xs">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <div class="text-sm font-extrabold text-slate-900 dark:text-white">{{ number_format($totalMembers) }}</div>
                    <div class="text-[10px] text-slate-500 dark:text-slate-400 font-bold uppercase tracking-wider">Total Gym Members</div>
                </div>
            </div>
            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    <div class="text-sm font-extrabold text-slate-900 dark:text-white">{{ number_format($totalBranchesCount) }}</div>
                    <div class="text-[10px] text-slate-500 dark:text-slate-400 font-bold uppercase tracking-wider">Total Gym Branches</div>
                </div>
            </div>
            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    <div class="text-sm font-extrabold text-slate-900 dark:text-white">{{ $activeSubscriptionsCount }}</div>
                    <div class="text-[10px] text-slate-500 dark:text-slate-400 font-bold uppercase tracking-wider">Active Subscriptions</div>
                </div>
            </div>
            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    <div class="text-sm font-extrabold text-emerald-600 dark:text-emerald-400">100% Operational</div>
                    <div class="text-[10px] text-slate-500 dark:text-slate-400 font-bold uppercase tracking-wider">System Health</div>
                </div>
            </div>
        </div>
        ...
        <!-- Add Gym Modal -->
        <div x-show="showNewGymModal" class="fixed inset-0 z-50 overflow-y-auto bg-black/70 backdrop-blur-xs flex items-center justify-center p-4" x-cloak>
            <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl max-w-xl w-full p-6 shadow-2xl" @click.away="showNewGymModal = false">
                <div class="flex justify-between items-center mb-4 border-b border-slate-200 dark:border-slate-800 pb-3">
                    ...
                    <button @click="showNewGymModal = false" class="text-slate-400 hover:text-slate-600 dark:hover:text-white">✕</button>
                </div>
                ...
                <form action="{{ route('admin.gyms.store') }}" method="POST" class="space-y-4">
                    ...
                    <div class="grid grid-cols-4 gap-3">
                        <div>
                            ...
                            <select name="plan_id" class="w-full px-3 py-2 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-800 text-slate-900 dark:text-white text-xs focus:border-red-500 focus:outline-none">
                                @foreach($plans as $p)
                    ...
                    <div class="flex justify-end gap-3 pt-3 border-t border-slate-200 dark:border-slate-800">
                        <button type="button" @click="showNewGymModal = false" class="px-4 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 dark:bg-slate-800 dark:hover:bg-slate-700 dark:text-slate-300 text-xs font-semibold">Cancel</button>
                        <button type="submit" class="px-4 py-2 rounded-xl bg-red-500 hover:bg-red-600 text-white font-bold text-xs shadow-md shadow-red-500/20">Create Gym & Account</button>
                    </div>
                </form>
            </div>
        </div>
        ...
        <!-- Add Plan Modal -->
        <div x-show="showNewPlanModal" class="fixed inset-0 z-50 overflow-y-auto bg-black/70 backdrop-blur-xs flex items-center justify-center p-4" x-cloak>
            <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl max-w-xl w-full p-6 shadow-2xl" @click.away="showNewPlanModal = false">
                <div class="flex justify-between items-center mb-4 border-b border-slate-200 dark:border-slate-800 pb-3">
                    <h3 class="text-base font-bold text-slate-900 dark:text-white">Create SaaS Subscription Plan</h3>
                    <button @click="showNewPlanModal = false" class="text-slate-400 hover:text-slate-600 dark:hover:text-white">✕</button>
                </div>
                ...
                <form action="{{ route('admin.plans.store') }}" method="POST" class="space-y-4">
                    ...
                    <div class="flex justify-end gap-3 pt-3 border-t border-slate-200 dark:border-slate-800">
                        <button type="button" @click="showNewPlanModal = false" class="px-4 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 dark:bg-slate-800 dark:hover:bg-slate-700 dark:text-slate-300 text-xs font-semibold">Cancel</button>
                        <button type="submit" class="px-4 py-2 rounded-xl bg-red-500 hover:bg-red-600 text-white font-bold text-xs shadow-md shadow-red-500/20">Save SaaS Plan</button>
                    </div>
                </form>
            </div>
        </div>
